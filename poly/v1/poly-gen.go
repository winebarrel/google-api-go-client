...
const basePathTemplate = "https://poly.UNIVERSE_DOMAIN/"
...
// Asset: Represents and describes an asset in the Poly library. An asset is a
// 3D model or scene created using Tilt Brush (//www.tiltbrush.com), Blocks
// (//vr.google.com/blocks/), or any 3D program that produces a file that can
// be upload to Poly.
type Asset struct {
	// AuthorName: The author's publicly visible name. Use this name when giving
	// credit to the author. For more information, see Licensing
	// (/poly/discover/licensing).
	AuthorName string `json:"authorName,omitempty"`
	// CreateTime: For published assets, the time when the asset was published. For
	...
	// Formats: A list of Formats where each format describes one representation of
	// the asset.
	Formats []*Format `json:"formats,omitempty"`
	// IsCurated: Whether this asset has been curated by the Poly team.
	IsCurated bool `json:"isCurated,omitempty"`
	// License: The license under which the author has made the asset available for
	// use, if any.
	//
	// Possible values:
	...
	// Metadata: Application-defined opaque metadata for this asset. This field is
	// only returned when querying for the signed-in user's own assets, not for
	...
	// UpdateTime: The time when the asset was last modified. For published assets,
	// whose contents are immutable, the update time changes only when metadata
	...
	//   "PRIVATE" - Access to the asset and its underlying files and resources is
	// restricted to the author. **Authentication:** You must supply an OAuth token
	// that corresponds to the author's account.
	//   "UNLISTED" - Access to the asset and its underlying files and resources is
	// available to anyone with the asset's name. Unlisted assets are **not**
...
type AssetImportMessage struct {
	...
	// was found.
	//   "OBJ_PARSE_ERROR" - A problem was encountered while parsing the OBJ file.
	// The converter makes a 'best effort' attempt to continue when encountering
	// such issues. In some cases the resulting preview model may still be
	...
	//   "DEFAULT_MATERIALS" - Default materials are used in the model. This means
	// that one or more faces is using default materials either because no usemtl
	// statement was specified or because the requested material was not found due
	// to a missing material file or bad material name. This does not cover the
	// case of missing textures.
	...
	// FilePath: An optional file path. Only present for those error codes that
	// specify it.
	FilePath string `json:"filePath,omitempty"`
	// ImageError: An optional image error. Only present for INVALID_IMAGE_FILE.
...
// File: Represents a file in Poly, which can be a root, resource, or thumbnail
// file.
type File struct {
	...
	// RelativePath: The path of the resource file relative to the root file. For
	// root or thumbnail files, this is just the filename.
...
// Format: The same asset can be represented in different formats, for example,
...
type Format struct {
	...
	// representation. Possible values are: `FBX`, `GLTF`, `GLTF2`, `OBJ`, and
	// `TILT`.
	FormatType string `json:"formatType,omitempty"`
	// Resources: A list of dependencies of the root element. May include, but is
	// not limited to, materials, textures, and shader programs.
	Resources []*File `json:"resources,omitempty"`
	// Root: The root of the file hierarchy. This will always be populated. For
	// some format_types - such as `TILT`, which are self-contained - this is all
	...
	// details.
	ForceSendFields []string `json:"-"`
	// NullFields is a list of field names (e.g. "FormatComplexity") to include in
...
type FormatComplexity struct {
	// LodHint: A non-negative integer that represents the level of detail (LOD) of
	...
	// ForceSendFields is a list of field names (e.g. "LodHint") to unconditionally
	// include in API requests. By default, fields with empty or default values are
	// omitted from API requests. See
	// https://pkg.go.dev/google.golang.org/api#hdr-ForceSendFields for more
	// details.
	ForceSendFields []string `json:"-"`
	// NullFields is a list of field names (e.g. "LodHint") to include in API
	// requests with the JSON null value. By default, fields with empty values are
	// omitted from API requests. See
	// https://pkg.go.dev/google.golang.org/api#hdr-NullFields for more details.
	NullFields []string `json:"-"`
...
type ImageError struct {
	// Code: The type of image error encountered. Optional for older image errors.
	//
	// Possible values:
	//   "CODE_UNSPECIFIED" - Unknown error code.
	//   "INVALID_IMAGE" - We were unable to read the image file.
	//   "IMAGE_TOO_BIG" - The image size is too large.
	//   "WRONG_IMAGE_TYPE" - The image data does not match the expected MIME type
	// of the image.
	Code string `json:"code,omitempty"`
...
type ListAssetsResponse struct {
	...
	// NextPageToken: The continuation token for retrieving the next page. If
	// empty, indicates that there are no more pages. To get the next page, submit
	// the same request specifying this value as the page_token.
...
type ObjParseError struct {
	...
	//   "UNKNOWN_MATERIAL" - The specified material was not found in any material
	// definition in the import.
	...
	FilePath string `json:"filePath,omitempty"`
	// Line: The text of the line. Note that this may be truncated if the line was
	...
	// StartIndex: The starting character index at which the problem was found.
...
// PresentationParams: Hints for displaying the asset, based on information
// available when the asset was uploaded.
type PresentationParams struct {
	// BackgroundColor: A background color which could be used for displaying the
	// 3D asset in a 'thumbnail' or 'palette' style view. Authors have the option
	// to set this background color when publishing or editing their asset. This is
	// represented as a six-digit hexademical triplet specifying the RGB components
	// of the background color, e.g. #FF0000 for Red.
	BackgroundColor string `json:"backgroundColor,omitempty"`
	// ColorSpace: The materials' diffuse/albedo color. This does not apply to
	// vertex colors or texture maps.
	//
	// Possible values:
	...
	// OrientingRotation: A rotation that should be applied to the object root to
	// make it upright. More precisely, this quaternion transforms from "object
	// space" (the space in which the object is defined) to "presentation space", a
	// coordinate system where +Y is up, +X is right, -Z is forward. For example,
	// if the object is the Eiffel Tower, in its local coordinate system the object
	// might be laid out such that the base of the tower is on the YZ plane and the
	// tip of the tower is towards positive X. In this case this quaternion would
...
// Quaternion: A Quaternion (//en.wikipedia.org/wiki/Quaternion). Please note:
// if in the response you see "w: 1" and nothing else this is the default value
...
type Quaternion struct {
	...
	// ForceSendFields is a list of field names (e.g. "W") to unconditionally
	// include in API requests. By default, fields with empty or default values are
	// omitted from API requests. See
	// https://pkg.go.dev/google.golang.org/api#hdr-ForceSendFields for more
	// details.
	ForceSendFields []string `json:"-"`
	// NullFields is a list of field names (e.g. "W") to include in API requests
	// with the JSON null value. By default, fields with empty values are omitted
	// from API requests. See
	// https://pkg.go.dev/google.golang.org/api#hdr-NullFields for more details.
	NullFields []string `json:"-"`
...
// RemixInfo: Info about the sources of this asset (i.e. assets that were
// remixed to create this asset).
type RemixInfo struct {
	...
	// details.
	ForceSendFields []string `json:"-"`
	// NullFields is a list of field names (e.g. "SourceAsset") to include in API
	// requests with the JSON null value. By default, fields with empty values are
	// omitted from API requests. See
	// https://pkg.go.dev/google.golang.org/api#hdr-NullFields for more details.
	NullFields []string `json:"-"`
...
type StartAssetImportResponse struct {
	// AssetId: The id of newly created asset. If this is empty when the operation
	// is complete it means the import failed. Please refer to the
	...
	// AssetImportMessages: The message from the asset import. This will contain
	// any warnings (or - in the case of failure - errors) that occurred during
	// import.
...
// Get: Returns detailed information about an asset given its name. PRIVATE
...
func (c *AssetsGetCall) doRequest(alt string) (*http.Response, error) {
	...
	c.s.logger.DebugContext(c.ctx_, "api request", "serviceName", apiName, "rpcName", "poly.assets.get", "request", internallog.HTTPRequest(req, nil))
...
func (c *AssetsGetCall) Do(opts ...googleapi.CallOption) (*Asset, error) {
	...
	c.s.logger.DebugContext(c.ctx_, "api response", "serviceName", apiName, "rpcName", "poly.assets.get", "response", internallog.HTTPResponse(res, b))
	return ret, nil
}
...
// List: Lists all public, remixable assets. These are assets with an access
...
// Category sets the optional parameter "category": Filter assets based on the
// specified category. Supported values are: `animals`, `architecture`, `art`,
...
// Curated sets the optional parameter "curated": Return only assets that have
// been curated by the Poly team.
func (c *AssetsListCall) Curated(curated bool) *AssetsListCall {
	c.urlParams_.Set("curated", fmt.Sprint(curated))
	return c
}

// Format sets the optional parameter "format": Return only assets with the
// matching format. Acceptable values are: `BLOCKS`, `FBX`, `GLTF`, `GLTF2`,
// `OBJ`, `TILT`.
...
// Keywords sets the optional parameter "keywords": One or more search terms to
// be matched against all text that Poly has indexed for assets, which includes
// display_name, description, and tags. Multiple keywords should be separated
// by spaces.
...
// MaxComplexity sets the optional parameter "maxComplexity": Returns assets
// that are of the specified complexity or less. Defaults to COMPLEX. For
// example, a request for MEDIUM assets also includes SIMPLE assets.
//
// Possible values:
//
//	"COMPLEXITY_UNSPECIFIED" - No complexity specified. This is equivalent to
//
// omitting the filter.
//
...
// OrderBy sets the optional parameter "orderBy": Specifies an ordering for
// assets. Acceptable values are: `BEST`, `NEWEST`, `OLDEST`. Defaults to
// `BEST`, which ranks assets based on a combination of popularity and other
// features.
...
// assets to be returned. This value must be between `1` and `100`. Defaults to
// `20`.
...
// PageToken sets the optional parameter "pageToken": Specifies a continuation
// token from a previous search whose results were split into multiple pages.
// To get the next page, submit the same request specifying the value from
// next_page_token.
...
func (c *AssetsListCall) doRequest(alt string) (*http.Response, error) {
	...
	req.Header = reqHeaders
	c.s.logger.DebugContext(c.ctx_, "api request", "serviceName", apiName, "rpcName", "poly.assets.list", "request", internallog.HTTPRequest(req, nil))
	return gensupport.SendRequest(c.ctx_, c.s.client, req)
}
...
func (c *AssetsListCall) Do(opts ...googleapi.CallOption) (*ListAssetsResponse, error) {
	...
	c.s.logger.DebugContext(c.ctx_, "api response", "serviceName", apiName, "rpcName", "poly.assets.list", "response", internallog.HTTPResponse(res, b))
	return ret, nil
}
...
// assets to be returned. Defaults to VISIBILITY_UNSPECIFIED which returns all
// assets.
//
// Possible values:
//
...
func (c *UsersAssetsListCall) doRequest(alt string) (*http.Response, error) {
	...
	c.s.logger.DebugContext(c.ctx_, "api request", "serviceName", apiName, "rpcName", "poly.users.assets.list", "request", internallog.HTTPRequest(req, nil))
	return gensupport.SendRequest(c.ctx_, c.s.client, req)
}

// Do executes the "poly.users.assets.list" call.
// Any non-2xx status code is an error. Response headers are in either
// *ListUserAssetsResponse.ServerResponse.Header or (if a response was returned
...
func (c *UsersAssetsListCall) Do(opts ...googleapi.CallOption) (*ListUserAssetsResponse, error) {
	...
	c.s.logger.DebugContext(c.ctx_, "api response", "serviceName", apiName, "rpcName", "poly.users.assets.list", "response", internallog.HTTPResponse(res, b))
	return ret, nil
}
...
// OrderBy sets the optional parameter "orderBy": Specifies an ordering for
// assets. Acceptable values are: `BEST`, `NEWEST`, `OLDEST`, 'LIKED_TIME'.
// Defaults to `LIKED_TIME`, which ranks assets based on how recently they were
// liked.
...
func (c *UsersLikedassetsListCall) doRequest(alt string) (*http.Response, error) {
	...
	c.s.logger.DebugContext(c.ctx_, "api request", "serviceName", apiName, "rpcName", "poly.users.likedassets.list", "request", internallog.HTTPRequest(req, nil))
	return gensupport.SendRequest(c.ctx_, c.s.client, req)
}
...
func (c *UsersLikedassetsListCall) Do(opts ...googleapi.CallOption) (*ListLikedAssetsResponse, error) {
	...
	c.s.logger.DebugContext(c.ctx_, "api response", "serviceName", apiName, "rpcName", "poly.users.likedassets.list", "response", internallog.HTTPResponse(res, b))
	return ret, nil
}
