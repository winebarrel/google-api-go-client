...
type Authority struct {
	...
	// URI to validate tokens from the issuer. Clearing `issuer` disables Workload
...
// ConnectAgentName sets the optional parameter "connectAgent.name": Do not
// set.
...
// ConnectAgentNamespace sets the optional parameter "connectAgent.namespace":
// Namespace for GKE Connect agent resources. Defaults to `gke-connect`. The
// Connect Agent is authorized automatically when run in the default namespace.
// Otherwise, explicit authorization must be granted with an additional IAM
// binding.
...
// ConnectAgentProxy sets the optional parameter "connectAgent.proxy": URI of a
// proxy if connectivity from the agent to gkeconnect.googleapis.com requires
// the use of a proxy. Format must be in the form `http(s)://{proxy_address}`,
// depending on the HTTP/HTTPS protocol supported by the proxy. This will
// direct the connect agent's outbound traffic through a HTTP(S) proxy.
...
// fields to update. At least one field path must be specified in this mask.
