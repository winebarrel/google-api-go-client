...
const basePathTemplate = "https://analyticsreporting.UNIVERSE_DOMAIN/"
...
// Activity: An Activity represents data for an activity of a user. Note that
// an Activity is different from a hit. A hit might result in multiple
...
type Activity struct {
	// ActivityTime: Timestamp of the activity. If activities for a visit cross
	// midnight and occur in two separate dates, then two sessions (one per date)
	// share the session identifier. For example, say session ID 113472 has
	// activity within 2019-08-20, and session ID 243742 has activity within
	// 2019-08-25 and 2019-08-26. Session ID 113472 is one session, and session ID
	// 243742 is two sessions.
	...
	// Campaign: For manual campaign tracking, it is the value of the utm_campaign
	// campaign tracking parameter. For AdWords autotagging, it is the name(s) of
	// the online ad campaign(s) you use for the property. If you use neither, its
	// value is (not set).
	Campaign string `json:"campaign,omitempty"`
	// ChannelGrouping: The Channel Group associated with an end user's session for
	// this View (defined by the View's Channel Groupings).
	...
	// Event: This field contains all the details pertaining to an event and will
	// be set if `activity_type` equals `EVENT`.
	Event *EventData `json:"event,omitempty"`
	// Goals: This field contains a list of all the goals that were reached in this
	...
	// Keyword: For manual campaign tracking, it is the value of the utm_term
	// campaign tracking parameter. For AdWords traffic, it contains the best
	// matching targeting criteria. For the display network, where multiple
	// targeting criteria could have caused the ad to show up, it returns the best
	...
	// Medium: The type of referrals. For manual campaign tracking, it is the value
	// of the utm_medium campaign tracking parameter. For AdWords autotagging, it
	// is cpc. If users came from a search engine detected by Google Analytics, it
	// is organic. If the referrer is not a search engine, it is referral. If users
	// came directly to the property and document.referrer is empty, its value is
	// (none).
	Medium string `json:"medium,omitempty"`
	// Pageview: This will be set if `activity_type` equals `PAGEVIEW`. This field
	// contains all the details about the visitor and the page that was visited.
	Pageview *PageviewData `json:"pageview,omitempty"`
	// Source: The source of referrals. For manual campaign tracking, it is the
	// value of the utm_source campaign tracking parameter. For AdWords
	// autotagging, it is google. If you use neither, it is the domain of the
	// source (e.g., document.referrer) referring the users. It may also contain a
	...
	// NullFields is a list of field names (e.g. "ActivityTime") to include in API
...
// Cohort: Defines a cohort. A cohort is a group of users who share a common
// characteristic. For example, all users with the same acquisition date belong
// to the same cohort.
type Cohort struct {
	// DateRange: This is used for `FIRST_VISIT_DATE` cohort, the cohort selects
	// users whose first visit date is between start date and end date defined in
	// the DateRange. The date ranges should be aligned for cohort requests. If the
...
// "cohorts": [{ "name": "cohort 1", "type": "FIRST_VISIT_DATE", "dateRange": {
// "startDate": "2015-08-01", "endDate": "2015-08-01" } },{ "name": "cohort 2"
// "type": "FIRST_VISIT_DATE" "dateRange": { "startDate": "2015-07-01",
// "endDate": "2015-07-01" } }] }
type CohortGroup struct {
	// Cohorts: The definition for the cohort.
	Cohorts []*Cohort `json:"cohorts,omitempty"`
	// LifetimeValue: Enable Life Time Value (LTV). LTV measures lifetime value for
	// users acquired through different channels. Please see: Cohort Analysis
	// (https://support.google.com/analytics/answer/6074676) and Lifetime Value
	// (https://support.google.com/analytics/answer/6182550) If the value of
	...
	// and month boundaries. - The `viewId` must be an app view ID
	// (https://support.google.com/analytics/answer/2649553#WebVersusAppViews)
	LifetimeValue bool `json:"lifetimeValue,omitempty"`
	// ForceSendFields is a list of field names (e.g. "Cohorts") to unconditionally
	// include in API requests. By default, fields with empty or default values are
	// omitted from API requests. See
	// https://pkg.go.dev/google.golang.org/api#hdr-ForceSendFields for more
	// details.
	ForceSendFields []string `json:"-"`
	// NullFields is a list of field names (e.g. "Cohorts") to include in API
	// requests with the JSON null value. By default, fields with empty values are
	// omitted from API requests. See
	// https://pkg.go.dev/google.golang.org/api#hdr-NullFields for more details.
	NullFields []string `json:"-"`
...
type CustomDimension struct {
	...
	// Value: Value of the custom dimension. Default value (i.e. empty string)
...
// endDate. The start and end dates are specified in ISO8601
// (https://en.wikipedia.org/wiki/ISO_8601) date format `YYYY-MM-DD`.
...
// DateRangeValues: Used to return a list of metrics for a single DateRange /
...
type DateRangeValues struct {
	...
	// ForceSendFields is a list of field names (e.g. "PivotValueRegions") to
	// unconditionally include in API requests. By default, fields with empty or
	// default values are omitted from API requests. See
	// https://pkg.go.dev/google.golang.org/api#hdr-ForceSendFields for more
	// details.
	ForceSendFields []string `json:"-"`
	// NullFields is a list of field names (e.g. "PivotValueRegions") to include in
...
// Dimension: Dimensions (https://support.google.com/analytics/answer/1033861)
// are attributes of your data. For example, the dimension `ga:city` indicates
// the city, for example, "Paris" or "New York", from which a session
// originates.
type Dimension struct {
	// HistogramBuckets: If non-empty, we place dimension values into buckets after
	// string to int64. Dimension values that are not the string representation of
	// an integral value will be converted to zero. The bucket values have to be in
	...
	// details.
	ForceSendFields []string `json:"-"`
	// NullFields is a list of field names (e.g. "HistogramBuckets") to include in
	// API requests with the JSON null value. By default, fields with empty values
	// are omitted from API requests. See
	// https://pkg.go.dev/google.golang.org/api#hdr-NullFields for more details.
	NullFields []string `json:"-"`
...
// DimensionFilter: Dimension filter specifies the filtering options on a
// dimension.
type DimensionFilter struct {
	// CaseSensitive: Should the match be case sensitive? Default is false.
	CaseSensitive bool `json:"caseSensitive,omitempty"`
	// DimensionName: The dimension to filter on. A DimensionFilter must contain a
	// dimension.
	DimensionName string `json:"dimensionName,omitempty"`
	// Expressions: Strings or regular expression to match against. Only the first
	// value of the list is used for comparison unless the operator is `IN_LIST`.
	// If `IN_LIST` operator, then the entire list is used to filter the dimensions
	...
	// Operator: How to match the dimension to the expression. The default is
	// REGEXP.
	//
	// Possible values:
	...
	//   "NUMERIC_EQUAL" - Integer comparison filters. case sensitivity is ignored
	// for these and the expression is assumed to be a string representing an
	// integer. Failure conditions: - If expression is not a valid int64, the
	// client should expect an error. - Input dimensions that are not valid int64
	...
	// restrictions.
	//   "NUMERIC_LESS_THAN" - Checks if the dimension is numerically less than the
	// match expression. Read the description for `NUMERIC_EQUALS` for
	// restrictions.
	...
	// for every single response row. For example: expressions: ["A", "B", "C"] Any
	// response row whose dimension has it is value as A, B or C, matches this
	// DimensionFilter.
...
// DimensionFilterClause: A group of dimension filters. Set the operator value
// to specify how the filters are logically combined.
type DimensionFilterClause struct {
	// Filters: The repeated set of filters. They are logically combined based on
...
// DynamicSegment: Dynamic segment definition for defining the segment within
// the request. A segment can select users, sessions or both.
...
type EventData struct {
	...
	// EventCategory: The object on the page that was interacted with. Eg: 'Video'.
	...
	// details.
	ForceSendFields []string `json:"-"`
	// NullFields is a list of field names (e.g. "EventAction") to include in API
	// requests with the JSON null value. By default, fields with empty values are
	// omitted from API requests. See
	// https://pkg.go.dev/google.golang.org/api#hdr-NullFields for more details.
	NullFields []string `json:"-"`
...
type GetReportsRequest struct {
	// ReportRequests: Requests, each request will have a separate response. There
	// can be a maximum of 5 requests. All requests should have the same
	// `dateRanges`, `viewId`, `segments`, `samplingLevel`, and `cohortGroup`.
	ReportRequests []*ReportRequest `json:"reportRequests,omitempty"`
	// UseResourceQuotas: Enables resource based quotas
	// (/analytics/devguides/reporting/core/v4/limits-quotas#analytics_reporting_api
	// _v4), (defaults to `False`). If this field is set to `True` the per view
	// (profile) quotas are governed by the computational cost of the request. Note
	// that using cost based quotas will higher enable sampling rates. (10 Million
	// for `SMALL`, 100M for `LARGE`. See the limits and quotas documentation
	// (/analytics/devguides/reporting/core/v4/limits-quotas#analytics_reporting_api
	// _v4) for details.
	...
	// details.
	ForceSendFields []string `json:"-"`
	// NullFields is a list of field names (e.g. "ReportRequests") to include in
	// API requests with the JSON null value. By default, fields with empty values
	// are omitted from API requests. See
	// https://pkg.go.dev/google.golang.org/api#hdr-NullFields for more details.
	NullFields []string `json:"-"`
...
// GetReportsResponse: The main response class which holds the reports from the
// Reporting API `batchGet` call.
type GetReportsResponse struct {
	// QueryCost: The amount of resource quota tokens deducted to execute the
	// query. Includes all responses.
	QueryCost int64 `json:"queryCost,omitempty"`
	// Reports: Responses corresponding to each of the request.
	Reports []*Report `json:"reports,omitempty"`
	// ResourceQuotasRemaining: The amount of resource quota remaining for the
	// property.
	...
	// details.
	ForceSendFields []string `json:"-"`
	// NullFields is a list of field names (e.g. "QueryCost") to include in API
	// requests with the JSON null value. By default, fields with empty values are
	// omitted from API requests. See
	// https://pkg.go.dev/google.golang.org/api#hdr-NullFields for more details.
	NullFields []string `json:"-"`
...
type GoalData struct {
	// GoalCompletionLocation: URL of the page where this goal was completed.
	...
	// GoalPreviousStep1: URL of the page one step prior to the goal completion.
	GoalPreviousStep1 string `json:"goalPreviousStep1,omitempty"`
	// GoalPreviousStep2: URL of the page two steps prior to the goal completion.
	GoalPreviousStep2 string `json:"goalPreviousStep2,omitempty"`
	// GoalPreviousStep3: URL of the page three steps prior to the goal completion.
	...
	// ForceSendFields is a list of field names (e.g. "GoalCompletionLocation") to
	// unconditionally include in API requests. By default, fields with empty or
	// default values are omitted from API requests. See
	// https://pkg.go.dev/google.golang.org/api#hdr-ForceSendFields for more
	// details.
	ForceSendFields []string `json:"-"`
	// NullFields is a list of field names (e.g. "GoalCompletionLocation") to
...
type GoalSetData struct {
	...
	// ForceSendFields is a list of field names (e.g. "Goals") to unconditionally
	// include in API requests. By default, fields with empty or default values are
	// omitted from API requests. See
	// https://pkg.go.dev/google.golang.org/api#hdr-ForceSendFields for more
	// details.
...
// Metric: Metrics (https://support.google.com/analytics/answer/1033861) are
// the quantitative measurements. For example, the metric `ga:users` indicates
// the total number of users for the requested time period.
type Metric struct {
	// Alias: An alias for the metric expression is an alternate name for the
	...
	// Expression: A metric expression in the request. An expression is constructed
	// from one or more metrics and numbers. Accepted operators include: Plus (+),
	// Minus (-), Negation (Unary -), Divided by (/), Multiplied by (*),
	// Parenthesis, Positive cardinal numbers (0-9), can include decimals and is
	// limited to 1024 characters. Example `ga:totalRefunds/ga:users`, in most
	// cases the metric expression is just a single metric name like `ga:users`.
	// Adding mixed `MetricType` (E.g., `CURRENCY` + `PERCENTAGE`) metrics will
	// result in unexpected results.
	Expression string `json:"expression,omitempty"`
	// FormattingType: Specifies how the metric expression should be formatted, for
	// example `INTEGER`.
	//
	// Possible values:
...
type MetricFilter struct {
	...
	// MetricName: The metric that will be filtered on. A metricFilter must contain
	// a metric name. A metric name can be an alias earlier defined as a metric or
	// it can also be a metric expression.
	MetricName string `json:"metricName,omitempty"`
	// Not: Logical `NOT` operator. If this boolean is set to true, then the
	// matching metric values will be excluded in the report. The default is false.
	Not bool `json:"not,omitempty"`
	// Operator: Is the metric `EQUAL`, `LESS_THAN` or `GREATER_THAN` the
	// comparisonValue, the default is `EQUAL`. If the operator is `IS_MISSING`,
	...
	// details.
	ForceSendFields []string `json:"-"`
	// NullFields is a list of field names (e.g. "ComparisonValue") to include in
	// API requests with the JSON null value. By default, fields with empty values
	// are omitted from API requests. See
	// https://pkg.go.dev/google.golang.org/api#hdr-NullFields for more details.
	NullFields []string `json:"-"`
...
// MetricFilterClause: Represents a group of metric filters. Set the operator
...
type MetricHeader struct {
	...
	// ForceSendFields is a list of field names (e.g. "MetricHeaderEntries") to
	// unconditionally include in API requests. By default, fields with empty or
	// default values are omitted from API requests. See
	// https://pkg.go.dev/google.golang.org/api#hdr-ForceSendFields for more
	// details.
	ForceSendFields []string `json:"-"`
	// NullFields is a list of field names (e.g. "MetricHeaderEntries") to include
...
type OrFiltersForSegment struct {
	// SegmentFilterClauses: List of segment filters to be combined with a `OR`
	// operator.
	SegmentFilterClauses []*SegmentFilterClause `json:"segmentFilterClauses,omitempty"`
	// ForceSendFields is a list of field names (e.g. "SegmentFilterClauses") to
	// unconditionally include in API requests. By default, fields with empty or
	// default values are omitted from API requests. See
	// https://pkg.go.dev/google.golang.org/api#hdr-ForceSendFields for more
	// details.
	ForceSendFields []string `json:"-"`
	// NullFields is a list of field names (e.g. "SegmentFilterClauses") to include
	// in API requests with the JSON null value. By default, fields with empty
	// values are omitted from API requests. See
	// https://pkg.go.dev/google.golang.org/api#hdr-NullFields for more details.
	NullFields []string `json:"-"`
...
type OrderBy struct {
	// FieldName: The field which to sort by. The default sort order is ascending.
	// Example: `ga:browser`. Note, that you can only specify one field for sort
	// here. For example, `ga:browser, ga:city` is not valid.
	...
	// based on value.
	//   "VALUE" - The sort order is based on the value of the chosen column; looks
	// only at the first date range.
	...
	// If column has n/d format, then weighted value of this ratio will be `(n +
	// totals.n)/(d + totals.d)` Usable only for metrics that represent ratios.
	//   "HISTOGRAM_BUCKET" - Histogram order type is applicable only to dimension
	// columns with non-empty histogram-buckets.
...
type PageviewData struct {
	...
	// details.
	ForceSendFields []string `json:"-"`
	// NullFields is a list of field names (e.g. "PagePath") to include in API
	// requests with the JSON null value. By default, fields with empty values are
	// omitted from API requests. See
	// https://pkg.go.dev/google.golang.org/api#hdr-NullFields for more details.
	NullFields []string `json:"-"`
...
// Pivot: The Pivot describes the pivot section in the request. The Pivot helps
// rearrange the information in the table for certain reports by pivoting your
// data on a second dimension.
type Pivot struct {
	// DimensionFilterClauses: DimensionFilterClauses are logically combined with
	// an `AND` operator: only data that is included by all these
	// DimensionFilterClauses contributes to the values in this pivot region.
	...
	// Dimensions: A list of dimensions to show as pivot columns. A Pivot can have
	// a maximum of 4 dimensions. Pivot dimensions are part of the restriction on
	// the total number of dimensions allowed in the request.
	Dimensions []*Dimension `json:"dimensions,omitempty"`
	// MaxGroupCount: Specifies the maximum number of groups to return. The default
	// value is 10, also the maximum value is 1,000.
	MaxGroupCount int64 `json:"maxGroupCount,omitempty"`
	// Metrics: The pivot metrics. Pivot metrics are part of the restriction on
	// total number of metrics allowed in the request.
	Metrics []*Metric `json:"metrics,omitempty"`
	// StartGroup: If k metrics were requested, then the response will contain some
	...
	// ForceSendFields is a list of field names (e.g. "DimensionFilterClauses") to
	// unconditionally include in API requests. By default, fields with empty or
	// default values are omitted from API requests. See
	// https://pkg.go.dev/google.golang.org/api#hdr-ForceSendFields for more
	// details.
	ForceSendFields []string `json:"-"`
	// NullFields is a list of field names (e.g. "DimensionFilterClauses") to
	// include in API requests with the JSON null value. By default, fields with
	// empty values are omitted from API requests. See
	// https://pkg.go.dev/google.golang.org/api#hdr-NullFields for more details.
	NullFields []string `json:"-"`
...
// PivotHeader: The headers for each of the pivot sections defined in the
// request.
type PivotHeader struct {
	...
	// ForceSendFields is a list of field names (e.g. "PivotHeaderEntries") to
	// unconditionally include in API requests. By default, fields with empty or
	// default values are omitted from API requests. See
	// https://pkg.go.dev/google.golang.org/api#hdr-ForceSendFields for more
	// details.
	ForceSendFields []string `json:"-"`
	// NullFields is a list of field names (e.g. "PivotHeaderEntries") to include
	// in API requests with the JSON null value. By default, fields with empty
	// values are omitted from API requests. See
	// https://pkg.go.dev/google.golang.org/api#hdr-NullFields for more details.
	NullFields []string `json:"-"`
...
type PivotHeaderEntry struct {
	...
	// details.
	ForceSendFields []string `json:"-"`
	// NullFields is a list of field names (e.g. "DimensionNames") to include in
	// API requests with the JSON null value. By default, fields with empty values
	// are omitted from API requests. See
	// https://pkg.go.dev/google.golang.org/api#hdr-NullFields for more details.
	NullFields []string `json:"-"`
...
type ProductData struct {
	...
	// ProductQuantity: Total number of this product units in the transaction.
	...
	// details.
	ForceSendFields []string `json:"-"`
	// NullFields is a list of field names (e.g. "ItemRevenue") to include in API
	// requests with the JSON null value. By default, fields with empty values are
	// omitted from API requests. See
	// https://pkg.go.dev/google.golang.org/api#hdr-NullFields for more details.
	NullFields []string `json:"-"`
...
type Report struct {
	...
	// details.
	ForceSendFields []string `json:"-"`
	// NullFields is a list of field names (e.g. "ColumnHeader") to include in API
	// requests with the JSON null value. By default, fields with empty values are
	// omitted from API requests. See
	// https://pkg.go.dev/google.golang.org/api#hdr-NullFields for more details.
	NullFields []string `json:"-"`
...
type ReportData struct {
	// DataLastRefreshed: The last time the data in the report was refreshed. All
	// the hits received before this timestamp are included in the calculation of
	// the report.
	...
	// IsDataGolden: Indicates if response to this request is golden or not. Data
	// is golden when the exact same request will not produce any new results if
	// asked at a later point in time.
	IsDataGolden bool `json:"isDataGolden,omitempty"`
	// Maximums: Minimum and maximum values seen over all matching rows. These are
	// both empty when `hideValueRanges` in the request is false, or when rowCount
	// is zero.
	Maximums []*DateRangeValues `json:"maximums,omitempty"`
	// Minimums: Minimum and maximum values seen over all matching rows. These are
	// both empty when `hideValueRanges` in the request is false, or when rowCount
	// is zero.
	...
	// SamplesReadCounts: If the results are sampled
	// (https://support.google.com/analytics/answer/2637192), this returns the
	// total number of samples read, one entry per date range. If the results are
	// not sampled this field will not be defined. See developer guide
	// (/analytics/devguides/reporting/core/v4/basics#sampling) for details.
	SamplesReadCounts googleapi.Int64s `json:"samplesReadCounts,omitempty"`
	// SamplingSpaceSizes: If the results are sampled
	// (https://support.google.com/analytics/answer/2637192), this returns the
	// total number of samples present, one entry per date range. If the results
	// are not sampled this field will not be defined. See developer guide
	// (/analytics/devguides/reporting/core/v4/basics#sampling) for details.
	...
	// the query, every requested value format gets a total. The total for a value
	...
	// ForceSendFields is a list of field names (e.g. "DataLastRefreshed") to
	// unconditionally include in API requests. By default, fields with empty or
	// default values are omitted from API requests. See
	// https://pkg.go.dev/google.golang.org/api#hdr-ForceSendFields for more
	// details.
	ForceSendFields []string `json:"-"`
	// NullFields is a list of field names (e.g. "DataLastRefreshed") to include in
	// API requests with the JSON null value. By default, fields with empty values
	// are omitted from API requests. See
	// https://pkg.go.dev/google.golang.org/api#hdr-NullFields for more details.
	NullFields []string `json:"-"`
...
// ReportRequest: The main request class which specifies the Reporting API
// request.
type ReportRequest struct {
	// CohortGroup: Cohort group associated with this request. If there is a cohort
	// group in the request the `ga:cohort` dimension must be present. Every
	...
	// DateRanges: Date ranges in the request. The request can have a maximum of 2
	// date ranges. The response will contain a set of metric values for each
	...
	// the same `dateRanges` definition.
	DateRanges []*DateRange `json:"dateRanges,omitempty"`
	// DimensionFilterClauses: The dimension filter clauses for filtering Dimension
	// Values. They are logically combined with the `AND` operator. Note that
	// filtering occurs before any dimensions are aggregated, so that the returned
	// metrics represent the total for only the relevant dimensions.
	DimensionFilterClauses []*DimensionFilterClause `json:"dimensionFilterClauses,omitempty"`
	...
	// dimension or metric on which to filter, followed by the filter expression.
	// For example, the following expression selects `ga:browser` dimension which
	// starts with Firefox; `ga:browser=~^Firefox`. For more information on
	// dimensions and metric filters, see Filters reference
	// (https://developers.google.com/analytics/devguides/reporting/core/v3/reference#filters).
	FiltersExpression string `json:"filtersExpression,omitempty"`
	// HideTotals: If set to true, hides the total of all metrics for all the
	// matching rows, for every date range. The default false and will return the
	// totals.
	HideTotals bool `json:"hideTotals,omitempty"`
	// HideValueRanges: If set to true, hides the minimum and maximum across all
	// matching rows. The default is false and the value ranges are returned.
	HideValueRanges bool `json:"hideValueRanges,omitempty"`
	// IncludeEmptyRows: If set to false, the response does not include rows if all
	// the retrieved metrics are equal to zero. The default is false which will
	// exclude these rows.
	IncludeEmptyRows bool `json:"includeEmptyRows,omitempty"`
	// MetricFilterClauses: The metric filter clauses. They are logically combined
	// with the `AND` operator. Metric filters look at only the first date range
	// and not the comparing date range. Note that filtering on metrics occurs
	// after the metrics are aggregated.
	MetricFilterClauses []*MetricFilterClause `json:"metricFilterClauses,omitempty"`
	// Metrics: The metrics requested. Requests must specify at least one metric.
	// Requests can have a total of 10 metrics.
	Metrics []*Metric `json:"metrics,omitempty"`
	// OrderBys: Sort order on output rows. To compare two rows, the elements of
	// the following are applied in order until a difference is found. All date
	// ranges in the output get the same row order.
	OrderBys []*OrderBy `json:"orderBys,omitempty"`
	// PageSize: Page size is for paging and specifies the maximum number of
	// returned rows. Page size should be >= 0. A query returns the default of
	// 1,000 rows. The Analytics Core Reporting API returns a maximum of 100,000
	// rows per request, no matter how many you ask for. It can also return fewer
	...
	// `ga:country`, so when segmenting only by country, you can't get more than
	// 300 rows, even if you set `pageSize` to a higher value.
	PageSize int64 `json:"pageSize,omitempty"`
	// PageToken: A continuation token to get the next page of the results. Adding
	// this to the request will return the rows after the pageToken. The pageToken
	// should be the value returned in the nextPageToken parameter in the response
	// to the GetReports request.
	PageToken string `json:"pageToken,omitempty"`
	// Pivots: The pivot definitions. Requests can have a maximum of 2 pivots.
	Pivots []*Pivot `json:"pivots,omitempty"`
	// SamplingLevel: The desired report sample
	// (https://support.google.com/analytics/answer/2637192) size. If the the
	// `samplingLevel` field is unspecified the `DEFAULT` sampling level is used.
	// Every ReportRequest (#ReportRequest) within a `batchGet` method must contain
	// the same `samplingLevel` definition. See developer guide
	// (/analytics/devguides/reporting/core/v4/basics#sampling) for details.
	//
	// Possible values:
	...
	// Segments: Segment the data returned for the request. A segment definition
	// helps look at a subset of the segment request. A request can contain up to
	// four segments. Every ReportRequest (#ReportRequest) within a `batchGet`
	// method must contain the same `segments` definition. Requests with segments
	// must have the `ga:segment` dimension.
	Segments []*Segment `json:"segments,omitempty"`
	// ViewId: The Analytics view ID
	// (https://support.google.com/analytics/answer/1009618) from which to retrieve
	// data. Every ReportRequest (#ReportRequest) within a `batchGet` method must
	...
	// details.
	ForceSendFields []string `json:"-"`
	// NullFields is a list of field names (e.g. "CohortGroup") to include in API
	// requests with the JSON null value. By default, fields with empty values are
	// omitted from API requests. See
	// https://pkg.go.dev/google.golang.org/api#hdr-NullFields for more details.
	NullFields []string `json:"-"`
...
type ResourceQuotasRemaining struct {
	...
	// ForceSendFields is a list of field names (e.g. "DailyQuotaTokensRemaining")
	// to unconditionally include in API requests. By default, fields with empty or
	// default values are omitted from API requests. See
	// https://pkg.go.dev/google.golang.org/api#hdr-ForceSendFields for more
	// details.
	ForceSendFields []string `json:"-"`
	// NullFields is a list of field names (e.g. "DailyQuotaTokensRemaining") to
	// include in API requests with the JSON null value. By default, fields with
	// empty values are omitted from API requests. See
	// https://pkg.go.dev/google.golang.org/api#hdr-NullFields for more details.
	NullFields []string `json:"-"`
...
type ScreenviewData struct {
	...
	// MobileDeviceBranding: Mobile manufacturer or branded name. Eg: "Google",
	// "Apple" etc.
...
// SearchUserActivityRequest: The request to fetch User Report from Reporting
// API `userActivity:get` call.
type SearchUserActivityRequest struct {
	// ActivityTypes: Set of all activity types being requested. Only acvities
	// matching these types will be returned in the response. If empty, all
	// activies will be returned.
	//
	// Possible values:
	//   "ACTIVITY_TYPE_UNSPECIFIED" - ActivityType will never have this value in
	// the response. Using this type in the request will result in an error.
	...
	// DateRange: Date range for which to retrieve the user activity. If a date
	// range is not provided, the default date range is (startDate: current date -
	// 7 days, endDate: current date - 1 day).
	DateRange *DateRange `json:"dateRange,omitempty"`
	// PageSize: Page size is for paging and specifies the maximum number of
	// returned rows. Page size should be > 0. If the value is 0 or if the field
	// isn't specified, the request returns the default of 1000 rows per page.
	PageSize int64 `json:"pageSize,omitempty"`
	// PageToken: A continuation token to get the next page of the results. Adding
	// this to the request will return the rows after the pageToken. The pageToken
	// should be the value returned in the nextPageToken parameter in the response
	// to the SearchUserActivityRequest (#SearchUserActivityRequest) request.
	PageToken string `json:"pageToken,omitempty"`
	// User: Required. Unique user Id to query for. Every SearchUserActivityRequest
	// (#SearchUserActivityRequest) must contain this field.
	User *User `json:"user,omitempty"`
	// ViewId: Required. The Analytics view ID
	// (https://support.google.com/analytics/answer/1009618) from which to retrieve
	// data. Every SearchUserActivityRequest (#SearchUserActivityRequest) must
	// contain the `viewId`.
	ViewId string `json:"viewId,omitempty"`
	...
	// details.
	ForceSendFields []string `json:"-"`
	// NullFields is a list of field names (e.g. "ActivityTypes") to include in API
	// requests with the JSON null value. By default, fields with empty values are
	// omitted from API requests. See
	// https://pkg.go.dev/google.golang.org/api#hdr-NullFields for more details.
	NullFields []string `json:"-"`
...
type SearchUserActivityResponse struct {
	// NextPageToken: This token should be passed to SearchUserActivityRequest
	// (#SearchUserActivityRequest) to retrieve the next page.
	NextPageToken string `json:"nextPageToken,omitempty"`
	// SampleRate: This field represents the sampling rate
	// (https://support.google.com/analytics/answer/2637192) for the given request
	// and is a number between 0.0 to 1.0. See developer guide
	// (/analytics/devguides/reporting/core/v4/basics#sampling) for details.
...
// Segment: The segment definition, if the report needs to be segmented. A
// Segment is a subset of the Analytics data. For example, of the entire set of
...
type Segment struct {
	...
	// SegmentId: The segment ID of a built-in or custom segment, for example
	// `gaid::-3`.
	...
	// details.
	ForceSendFields []string `json:"-"`
	// NullFields is a list of field names (e.g. "DynamicSegment") to include in
	// API requests with the JSON null value. By default, fields with empty values
	// are omitted from API requests. See
	// https://pkg.go.dev/google.golang.org/api#hdr-NullFields for more details.
	NullFields []string `json:"-"`
...
// SegmentDefinition: SegmentDefinition defines the segment to be a set of
// SegmentFilters which are combined together with a logical `AND` operation.
type SegmentDefinition struct {
	// SegmentFilters: A segment is defined by a set of segment filters which are
	...
	// details.
	ForceSendFields []string `json:"-"`
	// NullFields is a list of field names (e.g. "SegmentFilters") to include in
	// API requests with the JSON null value. By default, fields with empty values
	// are omitted from API requests. See
	// https://pkg.go.dev/google.golang.org/api#hdr-NullFields for more details.
	NullFields []string `json:"-"`
...
// SegmentDimensionFilter: Dimension filter specifies the filtering options on
// a dimension.
type SegmentDimensionFilter struct {
	// CaseSensitive: Should the match be case sensitive, ignored for `IN_LIST`
	// operator.
	CaseSensitive bool `json:"caseSensitive,omitempty"`
	// DimensionName: Name of the dimension for which the filter is being applied.
	DimensionName string `json:"dimensionName,omitempty"`
	// Expressions: The list of expressions, only the first element is used for all
	// operators
	Expressions []string `json:"expressions,omitempty"`
	...
	// Operator: The operator to use to match the dimension with the expressions.
	//
	// Possible values:
	//   "OPERATOR_UNSPECIFIED" - If the match type is unspecified, it is treated
	// as a REGEXP.
	...
	// ignored for these and the expression is assumed to be a string representing
	// an integer. Failure conditions: - if expression is not a valid int64, the
	// client should expect an error. - input dimensions that are not valid int64
	// values will never match the filter. Checks if the dimension is numerically
	// less than the match expression.
	//   "NUMERIC_GREATER_THAN" - Checks if the dimension is numerically greater
...
// SegmentFilter: SegmentFilter defines the segment to be either a simple or a
// sequence segment. A simple segment condition contains dimension and metric
// conditions to select the sessions or users. A sequence segment condition can
// be used to select users or sessions based on sequential conditions.
type SegmentFilter struct {
	// Not: If true, match the complement of simple or sequence segment. For
	// example, to match all visits not from "New York", we can define the segment
	// as follows: "sessionSegment": { "segmentFilters": [{ "simpleSegment" :{
	// "orFiltersForSegment": [{ "segmentFilterClauses":[{ "dimensionFilter": {
	// "dimensionName": "ga:city", "expressions": ["New York"] } }] }] }, "not":
	// "True" }] },
	Not bool `json:"not,omitempty"`
	// SequenceSegment: Sequence conditions consist of one or more steps, where
	// each step is defined by one or more dimension/metric conditions. Multiple
	// steps can be combined with special sequence operators.
	...
	// ForceSendFields is a list of field names (e.g. "Not") to unconditionally
	// include in API requests. By default, fields with empty or default values are
	// omitted from API requests. See
	// https://pkg.go.dev/google.golang.org/api#hdr-ForceSendFields for more
	// details.
	ForceSendFields []string `json:"-"`
	// NullFields is a list of field names (e.g. "Not") to include in API requests
	// with the JSON null value. By default, fields with empty values are omitted
	// from API requests. See
	// https://pkg.go.dev/google.golang.org/api#hdr-NullFields for more details.
	NullFields []string `json:"-"`
...
// SegmentFilterClause: Filter Clause to be used in a segment definition, can
// be wither a metric or a dimension filter.
...
type SegmentMetricFilter struct {
	// ComparisonValue: The value to compare against. If the operator is `BETWEEN`,
	...
	// contain a metric name.
	MetricName string `json:"metricName,omitempty"`
	// Operator: Specifies is the operation to perform to compare the metric. The
	// default is `EQUAL`.
	//
	// Possible values:
	...
	Operator string `json:"operator,omitempty"`
	// Scope: Scope for a metric defines the level at which that metric is defined.
...
type SegmentSequenceStep struct {
	// MatchType: Specifies if the step immediately precedes or can be any time
	...
	// OrFiltersForSegment: A sequence is specified with a list of Or grouped
	// filters which are combined with `AND` operator.
...
type SequenceSegment struct {
	// FirstStepShouldMatchFirstHit: If set, first step condition must match the
	// first hit of the visitor (in the date range).
	...
	// "FirstStepShouldMatchFirstHit") to unconditionally include in API requests.
	// By default, fields with empty or default values are omitted from API
	// requests. See https://pkg.go.dev/google.golang.org/api#hdr-ForceSendFields
	// for more details.
	ForceSendFields []string `json:"-"`
	// NullFields is a list of field names (e.g. "FirstStepShouldMatchFirstHit") to
	// include in API requests with the JSON null value. By default, fields with
	// empty values are omitted from API requests. See
	// https://pkg.go.dev/google.golang.org/api#hdr-NullFields for more details.
	NullFields []string `json:"-"`
...
type SimpleSegment struct {
	// OrFiltersForSegment: A list of segment filters groups which are combined
	// with logical `AND` operator.
	OrFiltersForSegment []*OrFiltersForSegment `json:"orFiltersForSegment,omitempty"`
	// ForceSendFields is a list of field names (e.g. "OrFiltersForSegment") to
	// unconditionally include in API requests. By default, fields with empty or
	// default values are omitted from API requests. See
	// https://pkg.go.dev/google.golang.org/api#hdr-ForceSendFields for more
	// details.
	ForceSendFields []string `json:"-"`
	// NullFields is a list of field names (e.g. "OrFiltersForSegment") to include
	// in API requests with the JSON null value. By default, fields with empty
	// values are omitted from API requests. See
	// https://pkg.go.dev/google.golang.org/api#hdr-NullFields for more details.
	NullFields []string `json:"-"`
...
// TransactionData: Represents details collected when the visitor performs a
// transaction on the page.
type TransactionData struct {
	// TransactionId: The transaction ID, supplied by the e-commerce tracking
	// method, for the purchase in the shopping cart.
	TransactionId string `json:"transactionId,omitempty"`
	// TransactionRevenue: The total sale revenue (excluding shipping and tax) of
	// the transaction.
...
type User struct {
	// Type: Type of the user in the request. The field `userId` is associated with
	// this type.
	//
	// Possible values:
	...
	// interact with content across one or more devices and / or browser instances.
...
// UserActivitySession: This represents a user session performed on a specific
// device at a certain time over a period of time.
type UserActivitySession struct {
	// Activities: Represents a detailed view into each of the activity in this
	// session.
	Activities []*Activity `json:"activities,omitempty"`
	// DataSource: The data source of a hit. By default, hits sent from
	// analytics.js are reported as "web" and hits sent from the mobile SDKs are
	// reported as "app". These values can be overridden in the Measurement
	// Protocol.
	DataSource string `json:"dataSource,omitempty"`
	// DeviceCategory: The type of device used: "mobile", "tablet" etc.
	DeviceCategory string `json:"deviceCategory,omitempty"`
	// Platform: Platform on which the activity happened: "android", "ios" etc.
...
func (c *ReportsBatchGetCall) doRequest(alt string) (*http.Response, error) {
	...
	c.s.logger.DebugContext(c.ctx_, "api request", "serviceName", apiName, "rpcName", "analyticsreporting.reports.batchGet", "request", internallog.HTTPRequest(req, body.Bytes()))
	return gensupport.SendRequest(c.ctx_, c.s.client, req)
}

// Do executes the "analyticsreporting.reports.batchGet" call.
// Any non-2xx status code is an error. Response headers are in either
// *GetReportsResponse.ServerResponse.Header or (if a response was returned at
...
func (c *ReportsBatchGetCall) Do(opts ...googleapi.CallOption) (*GetReportsResponse, error) {
	...
	c.s.logger.DebugContext(c.ctx_, "api response", "serviceName", apiName, "rpcName", "analyticsreporting.reports.batchGet", "response", internallog.HTTPResponse(res, b))
	return ret, nil
}
...
func (c *UserActivitySearchCall) doRequest(alt string) (*http.Response, error) {
	...
	req.Header = reqHeaders
	c.s.logger.DebugContext(c.ctx_, "api request", "serviceName", apiName, "rpcName", "analyticsreporting.userActivity.search", "request", internallog.HTTPRequest(req, body.Bytes()))
	return gensupport.SendRequest(c.ctx_, c.s.client, req)
}

// Do executes the "analyticsreporting.userActivity.search" call.
// Any non-2xx status code is an error. Response headers are in either
// *SearchUserActivityResponse.ServerResponse.Header or (if a response was
...
func (c *UserActivitySearchCall) Do(opts ...googleapi.CallOption) (*SearchUserActivityResponse, error) {
	...
	c.s.logger.DebugContext(c.ctx_, "api response", "serviceName", apiName, "rpcName", "analyticsreporting.userActivity.search", "response", internallog.HTTPResponse(res, b))
	return ret, nil
}
...
func (c *UserActivitySearchCall) Pages(ctx context.Context, f func(*SearchUserActivityResponse) error) error {
	c.ctx_ = ctx
	defer func(pt string) { c.searchuseractivityrequest.PageToken = pt }(c.searchuseractivityrequest.PageToken)
	for {
		x, err := c.Do()
		if err != nil {
			return err
