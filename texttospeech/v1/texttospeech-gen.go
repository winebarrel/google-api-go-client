...
type AudioConfig struct {
	...
	// Effects are applied on top of each other in the order they are given. See
	// audio profiles (https://cloud.google.com/text-to-speech/docs/audio-profiles)
	// for current supported profile ids.
	EffectsProfileId []string `json:"effectsProfileId,omitempty"`
	// Pitch: Optional. Input only. Speaking pitch, in the range [-20.0, 20.0]. 20
	// means increase 20 semitones from the original pitch. -20 means decrease 20
	// semitones from the original pitch.
	Pitch float64 `json:"pitch,omitempty"`
	// SampleRateHertz: Optional. The synthesis sample rate (in hertz) for this
	// audio. When this is specified in SynthesizeSpeechRequest, if this is
	...
	// unset, or set to a value of 0.0 (dB), will play at normal native signal
	...
	// amplitude. Strongly recommend not to exceed +10 (dB) as there's usually no
...
type CustomVoiceParams struct {
	...
	// ReportedUsage: Optional. Deprecated. The usage of the synthesized audio to
	// be reported.
	//
	// Possible values:
	//   "REPORTED_USAGE_UNSPECIFIED" - Request with reported usage unspecified
	// will be rejected.
	//   "REALTIME" - For scenarios where the synthesized audio is not downloadable
	// and can only be used once. For example, real-time request in IVR system.
	//   "OFFLINE" - For scenarios where the synthesized audio is downloadable and
	// can be reused. For example, the synthesized audio is downloaded, stored in
...
type GoogleCloudTexttospeechV1SynthesizeLongAudioMetadata struct {
	...
	// ProgressPercentage: The progress of the most recent processing update in
	// percentage, ie. 70.0%.
...
type ListVoicesResponse struct {
	...
	// ServerResponse contains the HTTP response code and headers from the server.
	googleapi.ServerResponse `json:"-"`
	// ForceSendFields is a list of field names (e.g. "Voices") to unconditionally
	// include in API requests. By default, fields with empty or default values are
	// omitted from API requests. See
	// https://pkg.go.dev/google.golang.org/api#hdr-ForceSendFields for more
	// details.
...
type SynthesizeSpeechResponse struct {
	// AudioContent: The audio data bytes encoded as specified in the request,
	// including the header for encodings that are wrapped in containers (e.g. MP3,
	// OGG_OPUS). For LINEAR16 audio, we include the WAV header. Note: as with all
...
type Voice struct {
	// LanguageCodes: The languages that this voice supports, expressed as BCP-47
	// (https://www.rfc-editor.org/rfc/bcp/bcp47.txt) language tags (e.g. "en-US",
	// "es-419", "cmn-tw").
	LanguageCodes []string `json:"languageCodes,omitempty"`
	// Name: The name of this voice. Each distinct voice has a unique name.
	Name string `json:"name,omitempty"`
	// NaturalSampleRateHertz: The natural sample rate (in hertz) for this voice.
	...
	// the selected voice will have. In the Voice field of ListVoicesResponse, this
	// may mean that the voice doesn't fit any of the other categories in this
	// enum, or that the gender of the voice isn't known.
	//   "MALE" - A male voice.
...
type VoiceSelectionParams struct {
	...
	// [CustomVoiceParams.model] is set, the service will choose the custom voice
	// matching the specified configuration.
	CustomVoice *CustomVoiceParams `json:"customVoice,omitempty"`
	// LanguageCode: Required. The language (and potentially also the region) of
	// the voice expressed as a BCP-47
	// (https://www.rfc-editor.org/rfc/bcp/bcp47.txt) language tag, e.g. "en-US".
	// This should not include a script tag (e.g. use "cmn-cn" rather than
	// "cmn-Hant-cn"), because the script will be inferred from the input provided
	// in the SynthesisInput. The TTS service will use this parameter to help
	// choose an appropriate voice. Note that the TTS service may choose a voice
	// with a slightly different language code than the one selected; it may
	// substitute a different region (e.g. using en-US rather than en-CA if there
	...
	// details.
	ForceSendFields []string `json:"-"`
	// NullFields is a list of field names (e.g. "CustomVoice") to include in API
	// requests with the JSON null value. By default, fields with empty values are
	// omitted from API requests. See
	// https://pkg.go.dev/google.golang.org/api#hdr-NullFields for more details.
	NullFields []string `json:"-"`
...
func (c *OperationsCancelCall) doRequest(alt string) (*http.Response, error) {
	...
	c.s.logger.DebugContext(c.ctx_, "api request", "serviceName", apiName, "rpcName", "texttospeech.operations.cancel", "request", internallog.HTTPRequest(req, body.Bytes()))
...
func (c *OperationsCancelCall) Do(opts ...googleapi.CallOption) (*Empty, error) {
	...
	c.s.logger.DebugContext(c.ctx_, "api response", "serviceName", apiName, "rpcName", "texttospeech.operations.cancel", "response", internallog.HTTPResponse(res, b))
	return ret, nil
}
...
func (c *OperationsDeleteCall) doRequest(alt string) (*http.Response, error) {
	...
	c.s.logger.DebugContext(c.ctx_, "api request", "serviceName", apiName, "rpcName", "texttospeech.operations.delete", "request", internallog.HTTPRequest(req, nil))
	return gensupport.SendRequest(c.ctx_, c.s.client, req)
}
...
func (c *OperationsDeleteCall) Do(opts ...googleapi.CallOption) (*Empty, error) {
	...
	c.s.logger.DebugContext(c.ctx_, "api response", "serviceName", apiName, "rpcName", "texttospeech.operations.delete", "response", internallog.HTTPResponse(res, b))
	return ret, nil
}
...
// Synthesize: Synthesizes speech synchronously: receive results after all text
...
// LanguageCode sets the optional parameter "languageCode": Recommended. BCP-47
// (https://www.rfc-editor.org/rfc/bcp/bcp47.txt) language tag. If not
...
// Any non-2xx status code is an error. Response headers are in either
// *ListVoicesResponse.ServerResponse.Header or (if a response was returned at
