...
type Pipeline struct {
	...
	// Each action can also specify its own encrypted environment. The secret must
	// decrypt to a JSON-encoded dictionary where key-value pairs serve as
	...
	EncryptedEnvironment *Secret `json:"encryptedEnvironment,omitempty"`
	// Environment: The environment to pass into every action. Each action can also
	...
	// pipeline fails to complete before the timeout, it will be cancelled and the
	// error code will be set to DEADLINE_EXCEEDED. If unspecified, it will default
	// to 7 days.
	Timeout string `json:"timeout,omitempty"`
...
// requesting user must have the `iam.serviceAccounts.actAs` permission for the
// Cloud Life Sciences service account or the request will fail.
type RunPipelineRequest struct {
	// Labels: User-defined labels to associate with the returned operation. These
	// labels are not propagated to any Google Cloud Platform resources used by the
	...
	Pipeline *Pipeline `json:"pipeline,omitempty"`
	// PubSubTopic: The name of an existing Pub/Sub topic. The server will publish
	// messages to this topic whenever the status of the operation changes. The
	// Life Sciences Service Agent account must have publisher permissions to the
...
type Secret struct {
	...
	// secret value. The VM service account must have the required permissions and
...
// container results in a non-zero exit status that was not otherwise ignored.
...
type VirtualMachine struct {
	...
	// large enough to accommodate all of the Docker images from each action in the
	// pipeline at the same time. If not specified, a small but reasonable default
	// value is used.
	...
	// platform can be allocated, but never one with fewer capabilities. The value
	// of this parameter must be a valid Compute Engine CPU platform name (such as
	// "Intel Skylake"). This parameter is only useful for carefully optimized work
	// loads where the CPU platform has a significant impact. For more information
	// about the effect of this parameter, see
	// https://cloud.google.com/compute/docs/instances/specify-min-cpu-platform.
	CpuPlatform string `json:"cpuPlatform,omitempty"`
	// Disks: The list of disks to create and attach to the VM. Specify either the
	// `volumes[]` field or the `disks[]` field, but not both.
	Disks []*Disk `json:"disks,omitempty"`
	// DockerCacheImages: The Compute Engine Disk Images to use as a Docker cache.
	...
	// images must match those of the tags used or the latest version will still be
	...
	// not cached will be stored on the first cache disk instead of the boot disk.
	...
	// (https://cloud.google.com/compute/docs/labeling-resources) on VM labels
	// imposed by Compute Engine. Labels keys with the prefix 'google-' are
	// reserved for use by Google. Labels applied at creation time to the VM.
	...
	// or a custom machine type (such as "custom-1-4096", where "1" indicates the
	// number of vCPUs and "4096" indicates the memory in MB). See Creating an
	// instance with a custom machine type
	...
	// NVIDIA GPU accelerator. The version specified here must be compatible with
	// the GPU libraries contained in the container being executed, and must be one
	// of the drivers hosted in the `nvidia-drivers-us-public` bucket on Google
	// Cloud Storage.
	NvidiaDriverVersion string `json:"nvidiaDriverVersion,omitempty"`
	// Preemptible: If true, allocate a preemptible VM.
	Preemptible bool `json:"preemptible,omitempty"`
	// Reservation: If specified, the VM will only be allocated inside the matching
	// reservation. It will fail if the VM parameters don't match the reservation.
	Reservation string `json:"reservation,omitempty"`
	// ServiceAccount: The service account to install on the VM. This account does
	// not need any permissions other than those required by the pipeline.
...
type Volume struct {
	...
	// details.
	ForceSendFields []string `json:"-"`
	// NullFields is a list of field names (e.g. "ExistingDisk") to include in API
	// requests with the JSON null value. By default, fields with empty values are
	// omitted from API requests. See
	// https://pkg.go.dev/google.golang.org/api#hdr-NullFields for more details.
	NullFields []string `json:"-"`
...
// to the pipeline has been released (deleted).
...
func (c *ProjectsLocationsGetCall) doRequest(alt string) (*http.Response, error) {
	...
	c.s.logger.DebugContext(c.ctx_, "api request", "serviceName", apiName, "rpcName", "lifesciences.projects.locations.get", "request", internallog.HTTPRequest(req, nil))
...
func (c *ProjectsLocationsGetCall) Do(opts ...googleapi.CallOption) (*Location, error) {
	...
	c.s.logger.DebugContext(c.ctx_, "api response", "serviceName", apiName, "rpcName", "lifesciences.projects.locations.get", "response", internallog.HTTPResponse(res, b))
	return ret, nil
}
...
func (c *ProjectsLocationsListCall) doRequest(alt string) (*http.Response, error) {
	...
	c.s.logger.DebugContext(c.ctx_, "api request", "serviceName", apiName, "rpcName", "lifesciences.projects.locations.list", "request", internallog.HTTPRequest(req, nil))
	return gensupport.SendRequest(c.ctx_, c.s.client, req)
}
...
func (c *ProjectsLocationsListCall) Do(opts ...googleapi.CallOption) (*ListLocationsResponse, error) {
	...
	c.s.logger.DebugContext(c.ctx_, "api response", "serviceName", apiName, "rpcName", "lifesciences.projects.locations.list", "response", internallog.HTTPResponse(res, b))
	return ret, nil
}
...
// Operations.ListOperations to check whether the cancellation succeeded or the
// operation completed despite cancellation. Authorization requires the
...
func (c *ProjectsLocationsOperationsCancelCall) doRequest(alt string) (*http.Response, error) {
	...
	c.s.logger.DebugContext(c.ctx_, "api request", "serviceName", apiName, "rpcName", "lifesciences.projects.locations.operations.cancel", "request", internallog.HTTPRequest(req, body.Bytes()))
	return gensupport.SendRequest(c.ctx_, c.s.client, req)
}
...
func (c *ProjectsLocationsOperationsCancelCall) Do(opts ...googleapi.CallOption) (*Empty, error) {
	...
	c.s.logger.DebugContext(c.ctx_, "api response", "serviceName", apiName, "rpcName", "lifesciences.projects.locations.operations.cancel", "response", internallog.HTTPResponse(res, b))
	return ret, nil
}
...
// service. Authorization requires the following Google IAM
// (https://cloud.google.com/iam) permission: * `lifesciences.operations.get`
//
...
func (c *ProjectsLocationsOperationsGetCall) doRequest(alt string) (*http.Response, error) {
	...
	c.s.logger.DebugContext(c.ctx_, "api request", "serviceName", apiName, "rpcName", "lifesciences.projects.locations.operations.get", "request", internallog.HTTPRequest(req, nil))
	return gensupport.SendRequest(c.ctx_, c.s.client, req)
}
...
func (c *ProjectsLocationsOperationsGetCall) Do(opts ...googleapi.CallOption) (*Operation, error) {
	...
	c.s.logger.DebugContext(c.ctx_, "api response", "serviceName", apiName, "rpcName", "lifesciences.projects.locations.operations.get", "response", internallog.HTTPResponse(res, b))
	return ret, nil
}
...
// (https://cloud.google.com/iam) permission: * `lifesciences.operations.list`
//
...
// Operations. The following filter fields are supported: * createTime: The
// time this job was created * events: The set of event (names) that have
// occurred while running the pipeline. The : operator can be used to determine
// if a particular event has occurred. * error: If the pipeline is running,
// this value is NULL. Once the pipeline finishes, the value is the standard
// Google error code. * labels.key or labels."key with space" where key is a
// label key. * done: If the pipeline is running, this value is false. Once the
...
// PageSize sets the optional parameter "pageSize": The maximum number of
// results to return. The maximum value is 256.
...
func (c *ProjectsLocationsOperationsListCall) doRequest(alt string) (*http.Response, error) {
	...
	c.s.logger.DebugContext(c.ctx_, "api request", "serviceName", apiName, "rpcName", "lifesciences.projects.locations.operations.list", "request", internallog.HTTPRequest(req, nil))
	return gensupport.SendRequest(c.ctx_, c.s.client, req)
}
...
func (c *ProjectsLocationsOperationsListCall) Do(opts ...googleapi.CallOption) (*ListOperationsResponse, error) {
	...
	c.s.logger.DebugContext(c.ctx_, "api response", "serviceName", apiName, "rpcName", "lifesciences.projects.locations.operations.list", "response", internallog.HTTPResponse(res, b))
	return ret, nil
}
...
// Run: Runs a pipeline. The returned Operation's metadata field will contain a
...
func (c *ProjectsLocationsPipelinesRunCall) doRequest(alt string) (*http.Response, error) {
	reqHeaders := gensupport.SetHeaders(c.s.userAgent(), "application/json", c.header_)
	body, err := googleapi.WithoutDataWrapper.JSONBuffer(c.runpipelinerequest)
	if err != nil {
		return nil, err
	}
	c.urlParams_.Set("alt", alt)
	...
	c.s.logger.DebugContext(c.ctx_, "api request", "serviceName", apiName, "rpcName", "lifesciences.projects.locations.pipelines.run", "request", internallog.HTTPRequest(req, body.Bytes()))
	return gensupport.SendRequest(c.ctx_, c.s.client, req)
}
...
func (c *ProjectsLocationsPipelinesRunCall) Do(opts ...googleapi.CallOption) (*Operation, error) {
	...
	c.s.logger.DebugContext(c.ctx_, "api response", "serviceName", apiName, "rpcName", "lifesciences.projects.locations.pipelines.run", "response", internallog.HTTPResponse(res, b))
	return ret, nil
}
