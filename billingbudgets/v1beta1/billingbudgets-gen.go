...
// GoogleCloudBillingBudgetsV1beta1AllUpdatesRule: AllUpdatesRule defines
...
type GoogleCloudBillingBudgetsV1beta1AllUpdatesRule struct {
	...
	// when a threshold is exceeded. This is in addition to default recipients who
	// have billing account IAM roles. The value is the full REST resource name of
	...
	// intervals to the topic. The topic needs to be created before the budget is
	// created; see
	...
	// The schema version of the notification sent to AllUpdatesRule.pubsub_topic.
...
// what you expect to spend on Cloud projects, plus the rules to execute as
// spend is tracked against that plan, (for example, send an alert when 90% of
// the target spend is met). The budget time period is configurable, with
// options such as month (default), quarter, year, or custom time period.
type GoogleCloudBillingBudgetsV1beta1Budget struct {
	...
	// and resource users, provided that they have the required IAM permissions.
	//   "BILLING_ACCOUNT" - Only billing account users have full access to the
	// `Budget`, resource-level users have read-only access, provided that they
	...
	// details.
	ForceSendFields []string `json:"-"`
	// NullFields is a list of field names (e.g. "AllUpdatesRule") to include in
...
type GoogleCloudBillingBudgetsV1beta1CreateBudgetRequest struct {
	...
	// ForceSendFields is a list of field names (e.g. "Budget") to unconditionally
...
// scope of the cost to calculate.
type GoogleCloudBillingBudgetsV1beta1Filter struct {
	...
	// will track usage from April 1 to June 30, when the current calendar month is
	...
	// omitted, the report will include all labeled and unlabeled usage. An object
	...
	// the budget. If omitted, the report will include all usage for the billing
	...
	// the budget. If omitted, the report will include usage for all the services.
	...
	// name of the parent account, usage from the parent account will be included.
	// If omitted, the report will include usage from the parent account and all
...
// are no other options yet. Future configuration options will be described
...
type GoogleCloudBillingBudgetsV1beta1UpdateBudgetRequest struct {
	...
	// update. Read-only fields (such as `name`) cannot be changed. If this is not
...
// *GoogleCloudBillingBudgetsV1beta1Budget.ServerResponse.Header or (if a
...
// Any non-2xx status code is an error. Response headers are in either
// *GoogleCloudBillingBudgetsV1beta1ListBudgetsResponse.ServerResponse.Header
