...
type ApplicationInfo struct {
	// Emails: Optional but at least one of uris, emails or instruction must be
	...
	// Instruction: Optional but at least one of uris, emails or instruction must
	// be specified. Use this field to provide instructions, such as "Mail your
	...
	// Uris: Optional but at least one of uris, emails or instruction must be
	// specified. Use this URI field to direct an applicant to a website, for
	// example to link to an online application form. The maximum number of allowed
...
type BucketizedCount struct {
	...
	// Range: Bucket range on which histogram was performed for the numeric field,
...
// the quality of search and recommendation for the API clients, enabling the
...
type ClientEvent struct {
	...
	// ExtraInfo: Optional. Extra information about this event. Used for storing
	// information with no matching field in event payload, for example, user
	// application specific context or details. At most 20 keys are supported. The
	// maximum total size of all keys and values is 2 KB.
	ExtraInfo map[string]string `json:"extraInfo,omitempty"`
	// JobEvent: A event issued when a job seeker interacts with the application
	// that implements Cloud Talent Solution.
	JobEvent *JobEvent `json:"jobEvent,omitempty"`
	// ParentEventId: Optional. The event_id of an event that resulted in the
	// current event. For example, a Job view event usually follows a parent
	// impression event: A job seeker first does a search where a list of jobs
	// appears (impression). The job seeker then selects a result and views the
	// description of a particular job (Job view).
	ParentEventId string `json:"parentEventId,omitempty"`
	// RequestId: Required. A unique ID generated in the API responses. It can be
...
type CommuteFilter struct {
	// AllowImpreciseAddresses: Optional. If true, jobs without "precise" addresses
	// (street level addresses or GPS coordinates) might also be returned. For city
	// and coarser level addresses, text matching is used. If this field is set to
	// false or is not specified, only jobs that include precise addresses are
	// returned by Commute Search. Note: If `allow_imprecise_addresses` is set to
	// true, Commute Search is not able to calculate accurate commute times to jobs
	...
	// CommuteMethod: Required. The method of transportation for which to calculate
	// the commute time.
	//
	// Possible values:
	...
	// impact, represented as google.type.TimeOfDay in local time zone. Currently
	// traffic model is restricted to hour level resolution.
	DepartureTime *TimeOfDay `json:"departureTime,omitempty"`
	// RoadTraffic: Optional. Specifies the traffic density to use when calculating
	// commute time.
	//
	// Possible values:
	...
	// StartCoordinates: Required. The latitude and longitude of the location from
...
type CommuteInfo struct {
	...
	// from the query location. A duration of 0 seconds indicates that the job is
	// not reachable within the requested duration, but was returned as part of an
	// expanded query.
	TravelDuration string `json:"travelDuration,omitempty"`
...
type Company struct {
	// CareerSiteUri: Optional. The URI to employer's career site or careers page
	// on the employer's web site, for example, "https://careers.google.com".
	...
	// EeoText: Optional. Equal Employment Opportunity legal disclaimer text to be
	...
	// HeadquartersAddress: Optional. The street address of the company's main
	// headquarters, which may be different from the job location. The service
	// attempts to geolocate the provided address, and populates a more specific
	...
	// HiringAgency: Optional. Set to true if it is the hiring agency that post
	...
	// KeywordSearchableJobCustomAttributes: Optional. This field is deprecated.
	...
	// WebsiteUri: Optional. The URI representing the company's primary web site or
	// home page, for example, "https://www.google.com". The maximum number of
...
type CompanyDerivedInfo struct {
	...
	// resolved from Company.hq_location if provided.
...
type CompensationEntry struct {
	...
	// Description: Optional. Compensation description. For example, could indicate
	// equity terms or provide additional context to an estimated bonus.
	Description string `json:"description,omitempty"`
	// ExpectedUnitsPerYear: Optional. Expected number of units paid each year. If
	// not specified, when Job.employment_types is FULLTIME, a default value is
...
type CompensationFilter struct {
	// IncludeJobsWithUnspecifiedCompensationRange: Optional. If set to true, jobs
	...
	// compensation_range. See CompensationInfo.CompensationEntry for definition of
...
type CompensationHistogramRequest struct {
	// BucketingOption: Required. Numeric histogram options, like buckets, whether
	// include min or max value.
	...
	//   "ANNUALIZED_TOTAL" - Histogram by job's annualized total compensation. See
	...
	// details.
	ForceSendFields []string `json:"-"`
	// NullFields is a list of field names (e.g. "BucketingOption") to include in
...
type CompensationInfo struct {
	...
	// range. Computed as base compensation entry's CompensationEntry.compensation
	// times CompensationEntry.expected_units_per_year. See CompensationEntry for
	...
	// range. Computed as all compensation entries' CompensationEntry.compensation
	// times CompensationEntry.expected_units_per_year. See CompensationEntry for
	// explanation on compensation annualization.
	AnnualizedTotalCompensationRange *CompensationRange `json:"annualizedTotalCompensationRange,omitempty"`
	// Entries: Optional. Job compensation information. At most one entry can be of
...
type CompensationRange struct {
	// MaxCompensation: Optional. The maximum amount of compensation. If left
	// empty, the value is set to a maximal compensation value and the currency
	// code is set to match the currency code of min_compensation.
	MaxCompensation *Money `json:"maxCompensation,omitempty"`
	// MinCompensation: Optional. The minimum amount of compensation. If left
	// empty, the value is set to zero and the currency code is set to match the
...
type CompletionResult struct {
	// ImageUri: The URI of the company image for CompletionType.COMPANY_NAME.
	ImageUri string `json:"imageUri,omitempty"`
...
type CreateClientEventRequest struct {
	// ClientEvent: Required. Events issued when end user interacts with customer's
	...
	// details.
	ForceSendFields []string `json:"-"`
	// NullFields is a list of field names (e.g. "ClientEvent") to include in API
	// requests with the JSON null value. By default, fields with empty values are
	// omitted from API requests. See
	// https://pkg.go.dev/google.golang.org/api#hdr-NullFields for more details.
	NullFields []string `json:"-"`
...
type CreateCompanyRequest struct {
	...
	// ForceSendFields is a list of field names (e.g. "Company") to unconditionally
	// include in API requests. By default, fields with empty or default values are
	// omitted from API requests. See
	// https://pkg.go.dev/google.golang.org/api#hdr-ForceSendFields for more
	// details.
	ForceSendFields []string `json:"-"`
	// NullFields is a list of field names (e.g. "Company") to include in API
	// requests with the JSON null value. By default, fields with empty values are
	// omitted from API requests. See
	// https://pkg.go.dev/google.golang.org/api#hdr-NullFields for more details.
	NullFields []string `json:"-"`
...
type CustomAttribute struct {
	// Filterable: Optional. If the `filterable` flag is true, the custom field
	// values may be used for custom attribute filters
	// JobQuery.custom_attribute_filter. If false, these values may not be used for
	// custom attribute filters. Default is false.
	Filterable bool `json:"filterable,omitempty"`
	// LongValues: Optional but exactly one of string_values or long_values must be
	...
	// (`CASE_SENSITIVE_MATCH` or `CASE_INSENSITIVE_MATCH`) search. For filterable
	// `string_value`s, a maximum total number of 200 values is allowed, with each
	// `string_value` has a byte size of no more than 500B. For unfilterable
	// `string_values`, the maximum total byte size of unfilterable `string_values`
...
// error is thrown if neither string_value_histogram or
// long_value_histogram_bucketing_option has been defined.
type CustomAttributeHistogramRequest struct {
	// Key: Required. Specifies the custom field key to perform a histogram on. If
	...
	// LongValueHistogramBucketingOption: Optional. Specifies buckets used to
	// perform a range histogram on Job's filterable long custom field values, or
	// min/max value requirements.
	LongValueHistogramBucketingOption *NumericBucketingOption `json:"longValueHistogramBucketingOption,omitempty"`
	// StringValueHistogram: Optional. If set to true, the response includes the
	// histogram value for each key as a string.
...
type CustomAttributeHistogramResult struct {
	// Key: Stores the key of custom attribute the histogram is performed on.
	Key string `json:"key,omitempty"`
	// LongValueHistogramResult: Stores bucketed histogram counting result or
	// min/max values for custom attribute long values associated with `key`.
	LongValueHistogramResult *NumericBucketingResult `json:"longValueHistogramResult,omitempty"`
	// StringValueHistogramResult: Stores a map from the values of string custom
	// field associated with `key` to the number of jobs with that value in this
	// histogram result.
...
type CustomRankingInfo struct {
	...
	//
	// Possible values:
	//   "IMPORTANCE_LEVEL_UNSPECIFIED" - Default value if the importance level is
	// not specified.
...
type DeviceInfo struct {
	...
	// Id: Optional. A device-specific ID. The ID must be a unique identifier that
...
type HistogramFacets struct {
	// CompensationHistogramFacets: Optional. Specifies compensation field-based
	// histogram requests. Duplicate values of CompensationHistogramRequest.type
	// are not allowed.
	CompensationHistogramFacets []*CompensationHistogramRequest `json:"compensationHistogramFacets,omitempty"`
	// CustomAttributeHistogramFacets: Optional. Specifies the custom attributes
	// histogram requests. Duplicate values of CustomAttributeHistogramRequest.key
	// are not allowed.
	CustomAttributeHistogramFacets []*CustomAttributeHistogramRequest `json:"customAttributeHistogramFacets,omitempty"`
	// SimpleHistogramFacets: Optional. Specifies the simple type of histogram
	// facets, for example, `COMPANY_SIZE`, `EMPLOYMENT_TYPE` etc.
	//
	// Possible values:
	...
	//   "ADMIN_1" - Filter by Admin1, which is a global placeholder for referring
	// to state, province, or the particular term a country uses to define the
	// geographic structure below the country level. Examples include states codes
	// such as "CA", "IL", "NY", and provinces, such as "BC".
	//   "COUNTRY" - Filter by the country code of job, such as US, JP, FR.
	//   "CITY" - Filter by the "city name", "Admin1 code", for example, "Mountain
	// View, CA" or "New York, NY".
	//   "LOCALE" - Filter by the locale field of a job, such as "en-US", "fr-FR".
	// This is the BCP-47 language code, such as "en-US" or "sr-Latn". For more
	// information, see [Tags for Identifying
	// Languages](https://tools.ietf.org/html/bcp47).
	//   "LANGUAGE" - Filter by the language code portion of the locale field, such
	// as "en" or "fr".
	//   "CATEGORY" - Filter by the Category.
	//   "CITY_COORDINATE" - Filter by the city center GPS coordinate (latitude and
	// longitude), for example, 37.4038522,-122.0987765. Since the coordinates of a
	// city center can change, clients may need to refresh them periodically.
	//   "ADMIN_1_COUNTRY" - A combination of state or province code with a country
	// code. This field differs from `JOB_ADMIN1`, which can be used in multiple
	// countries.
	...
	// ForceSendFields is a list of field names (e.g.
	// "CompensationHistogramFacets") to unconditionally include in API requests.
	// By default, fields with empty or default values are omitted from API
	// requests. See https://pkg.go.dev/google.golang.org/api#hdr-ForceSendFields
	// for more details.
	ForceSendFields []string `json:"-"`
	// NullFields is a list of field names (e.g. "CompensationHistogramFacets") to
	// include in API requests with the JSON null value. By default, fields with
	// empty values are omitted from API requests. See
	// https://pkg.go.dev/google.golang.org/api#hdr-NullFields for more details.
	NullFields []string `json:"-"`
...
type HistogramQuery struct {
	...
	// resources (for example, jobs) for searches. Expression syntax is a
	// aggregation function call with histogram facets and other options. Available
	// aggregation function calls are: * `count(string_histogram_facet)`: Count the
	// number of matching entity, for each distinct attribute value. *
	// `count(numeric_histogram_facet, list of buckets)`: Count the number of
	// matching entity within each bucket. A maximum of 200 histogram buckets are
	...
	// buckets in spec. * publish_time_in_month: histogram by the Job.publish_time
	// in months. Must specify list of numeric buckets in spec. *
	// publish_time_in_year: histogram by the Job.publish_time in years. Must
	// specify list of numeric buckets in spec. * degree_type: histogram by the
	// Job.degree_type. For example, "Bachelors", "Masters". * job_level: histogram
	// by the Job.job_level. For example, "Entry Level". * country: histogram by
	// the country code of jobs. For example, "US", "FR". * admin1: histogram by
	// the admin1 code of jobs, which is a global placeholder referring to the
	// state, province, or the particular term a country uses to define the
	// geographic structure below the country level. For example, "CA", "IL". *
	// city: histogram by a combination of the "city name, admin1 code". For
	// example, "Mountain View, CA", "New York, NY". * admin1_country: histogram by
	// a combination of the "admin1 code, country". For example, "CA, US", "IL,
	// US". * city_coordinate: histogram by the city center's GPS coordinates
	// (latitude and longitude). For example, 37.4038522,-122.0987765. Since the
	// coordinates of a city center can change, customers may need to refresh them
	...
	// For example, "WEEKLY", "MONTHLY". * base_compensation: histogram by the base
...
// contains the histogram map for the search type specified by
...
type HistogramResult struct {
	...
	// in this search result. Key: search type (filter names, such as the
	...
	// details.
	ForceSendFields []string `json:"-"`
	// NullFields is a list of field names (e.g. "SearchType") to include in API
	// requests with the JSON null value. By default, fields with empty values are
	// omitted from API requests. See
	// https://pkg.go.dev/google.golang.org/api#hdr-NullFields for more details.
	NullFields []string `json:"-"`
...
type HistogramResults struct {
	// CompensationHistogramResults: Specifies compensation field-based histogram
	// results that match HistogramFacets.compensation_histogram_requests.
	CompensationHistogramResults []*CompensationHistogramResult `json:"compensationHistogramResults,omitempty"`
	// CustomAttributeHistogramResults: Specifies histogram results for custom
	// attributes that match HistogramFacets.custom_attribute_histogram_facets.
	...
	// ForceSendFields is a list of field names (e.g.
	// "CompensationHistogramResults") to unconditionally include in API requests.
	// By default, fields with empty or default values are omitted from API
	// requests. See https://pkg.go.dev/google.golang.org/api#hdr-ForceSendFields
	// for more details.
	ForceSendFields []string `json:"-"`
	// NullFields is a list of field names (e.g. "CompensationHistogramResults") to
	// include in API requests with the JSON null value. By default, fields with
	// empty values are omitted from API requests. See
	// https://pkg.go.dev/google.golang.org/api#hdr-NullFields for more details.
	NullFields []string `json:"-"`
...
type Job struct {
	...
	// experience. Location(s) where the employer is looking to hire for this job
	// posting. Specifying the full street address(es) of the hiring location
	// enables better API results, especially job searches by commute time. At most
	// 50 locations are allowed for best search performance. If a job has more
	...
	// should be used for storage. It is also suggested to group the locations that
	// close to each other in the same job for better search experience. Jobs with
	// multiple addresses must have their addresses with the same LocationType to
	// allow location filtering to work properly. (For example, a Job with
	// addresses "1600 Amphitheatre Parkway, Mountain View, CA, USA" and "London,
	// UK" may not have location filters applied correctly at search time since the
	// first is a LocationType.STREET_ADDRESS and the second is a
	...
	// ApplicationInfo: Required. At least one field within ApplicationInfo must be
	// specified. Job application information.
	...
	// CompanyName: Required. The resource name of the company listing the job,
	// such as "projects/api-test-project/companies/foo".
	...
	// CustomAttributes: Optional. A map of fields to hold both filterable and
	// non-filterable custom job attributes that are not covered by the provided
	// structured fields. The keys of the map are strings up to 64 bytes and must
	// match the pattern: a-zA-Z*. For example, key0LikeThis or KEY_1_LIKE_THIS. At
	...
	// DegreeTypes: Optional. The desired education degrees for the job, such as
	// Bachelors, Masters.
	//
	// Possible values:
	...
	// Department: Optional. The department or functional area within the company
	// with the open position. The maximum number of allowed characters is 255.
	...
	// compensation incentives associated with the job not including salary or pay.
	...
	// JobEndTime: Optional. The end timestamp of the job. Typically this field is
	// used for contracting engagements. Invalid timestamps are ignored.
	JobEndTime string `json:"jobEndTime,omitempty"`
	// JobLevel: Optional. The experience level associated with the job, such as
	// "Entry Level".
	//
	// Possible values:
	...
	// JobStartTime: Optional. The start timestamp of the job in UTC time zone.
	// Typically this field is used for contracting engagements. Invalid timestamps
	// are ignored.
	JobStartTime string `json:"jobStartTime,omitempty"`
	// LanguageCode: Optional. The language of the posting. This field is distinct
	// from any requirements for fluency that are associated with the job. Language
	// codes must be in BCP-47 format, such as "en-US" or "sr-Latn". For more
	...
	// PostingExpireTime: Optional but strongly recommended for the best service
	// experience. The expiration timestamp of the job. After this timestamp, the
	// job is marked as expired, and it no longer appears in search results. The
	// expired job can't be deleted or listed by the DeleteJob and ListJobs APIs,
	// but it can be retrieved with the GetJob API or updated with the UpdateJob
	...
	// provided. Timestamp before the instant request is made is considered valid,
	// the job will be treated as expired immediately. If this value is not
	...
	// created on 2017/01/01 13:00AM UTC with an unspecified expiration date, the
	// job expires after 2017/01/31 13:00AM UTC. If this value is not provided on
	// job update, it depends on the field masks set by
	// UpdateJobRequest.update_mask. If the field masks include expiry_time, or the
	// masks are empty meaning that every field is updated, the job posting expires
	// after 30 days from the job's last update time. Otherwise the expiration date
	// isn't updated.
	...
	// recently published. The default value is the time the request arrives at the
	// server. Invalid timestamps are ignored.
	PostingPublishTime string `json:"postingPublishTime,omitempty"`
	// PostingRegion: Optional. The job PostingRegion (for example, state, country)
	...
	// PromotionValue: Optional. A promotion value of the job, as determined by the
	...
	// promotional values being returned first and ties being resolved by relevance
	// sort. Only the jobs with a promotionValue >0 are returned in a
	// FEATURED_JOB_SEARCH. Default value is 0, and negative values are treated as
	// 0.
	PromotionValue int64 `json:"promotionValue,omitempty"`
	// Qualifications: Optional. A description of the qualifications required to
	// perform the job. The use of this field is recommended as an alternative to
	// using the more general description field. This field accepts and sanitizes
	// HTML input, and also accepts bold, italic, ordered list, and unordered list
	...
	// ID, assigned by the client to identify a job. This field is intended to be
	// used by clients for client identification and tracking of postings. A job is
	// not allowed to be created if there is another job with the same
	// [company_name], language_code and requisition_id. The maximum number of
	// allowed characters is 255.
	RequisitionId string `json:"requisitionId,omitempty"`
	// Responsibilities: Optional. A description of job responsibilities. The use
	...
	//   "ACCOUNT_ONLY" - The resource is only visible to the GCP account who owns
	// it.
...
type JobQuery struct {
	...
	// different travel methods (for example, driving or public transit). Note:
	// This only works with COMMUTE MODE. When specified,
	...
	// Company.display_name of the jobs to search against. The company name must
	// match the value exactly. Alternatively, the value being searched for can be
	// wrapped in different match operators. `SUBSTRING_MATCH([value])` The company
	// name must contain a case insensitive substring match of the value. Using
	// this function may increase latency. Sample Value: `SUBSTRING_MATCH(google)`
	...
	// CompanyNames: Optional. This filter specifies the company entities to search
	// against. If a value isn't specified, jobs are searched for against all
	// companies. If multiple values are specified, jobs are searched against the
	// companies specified. The format is
	// "projects/{project_id}/companies/{company_id}", for example,
	// "projects/api-test-project/companies/foo". At most 20 company filters are
	// allowed.
	...
	// CustomAttributeFilter: Optional. This filter specifies a structured syntax
	// to match against the Job.custom_attributes marked as `filterable`. The
	// syntax for this expression is a subset of SQL syntax. Supported operators
	// are: `=`, `!=`, `<`, `<=`, `>`, and `>=` where the left of the operator is a
	// custom field key and the right of the operator is a number or a quoted
	// string. You must escape backslash (\\) and quote (\") characters. Supported
	// functions are `LOWER([field_name])` to perform a case insensitive match and
	...
	// DisableSpellCheck: Optional. This flag controls the spell-check feature. If
	// false, the service attempts to correct a misspelled query, for example,
	...
	// employment type of jobs to search against, such as EmploymentType.FULL_TIME.
	...
	// types.
	EmploymentTypes []string `json:"employmentTypes,omitempty"`
	// ExcludedJobs: Optional. This filter specifies a list of job names to be
	// excluded during search. At most 400 excluded job names are allowed.
	ExcludedJobs []string `json:"excludedJobs,omitempty"`
	// JobCategories: Optional. The category filter specifies the categories of
	// jobs to search against. See Category for more information. If a value is not
	...
	// as Truck Driver.
	JobCategories []string `json:"jobCategories,omitempty"`
	// LanguageCodes: Optional. This filter specifies the locale of jobs to search
	// against, for example, "en-US". If a value isn't specified, the search
	// results can contain jobs in any locale. Language codes should be in BCP-47
	...
	// information. If a location value isn't specified, jobs fitting the other
	// search criteria are retrieved regardless of where they're located. If
	// multiple values are specified, jobs are retrieved from any of the specified
	...
	// PublishTimeRange: Optional. Jobs published within a range specified by this
...
type Location struct {
	...
	// field of PostalAddress. For example, "Downtown, Atlanta, GA, USA" has a type
	// of LocationType#NEIGHBORHOOD, and "Kansas City, KS, USA" has a type of
	...
	// RadiusInMiles: Radius in miles of the job location. This value is derived
	// from the location bounding box in which a circle with the specified radius
	// centered from LatLng covers the area associated with the job location. For
...
type LocationFilter struct {
	// Address: Optional. The address name, such as "Mountain View" or "Bay Area".
	Address string `json:"address,omitempty"`
	// DistanceInMiles: Optional. The distance_in_miles is applied when the
	// location being searched for is identified as a city or smaller. When the
	// location being searched for is a state or larger, this field is ignored.
	DistanceInMiles float64 `json:"distanceInMiles,omitempty"`
	// LatLng: Optional. The latitude and longitude of the geographic center from
	// which to search. This field's ignored if `address` is provided.
	LatLng *LatLng `json:"latLng,omitempty"`
	// RegionCode: Optional. CLDR region code of the country/region. This field may
	// be used in two ways: 1) If telecommute preference is not set, this field is
	// used address ambiguity of the user-input address. For example, "Liverpool"
	// may refer to "Liverpool, NY, US" or "Liverpool, UK". This region code biases
	// the address resolution toward a specific country or territory. If this field
	// is not set, address resolution is biased toward the United States by
	// default. 2) If telecommute preference is set to TELECOMMUTE_ALLOWED, the
	...
	// TelecommutePreference: Optional. Allows the client to return jobs without a
	// set location, specifically, telecommuting jobs (telecommuting is considered
	// by the service as a special location). Job.posting_region indicates if a job
...
type MatchingJob struct {
	...
	// JobTitleSnippet: Contains snippets of text from the Job.job_title field most
...
// option for the histogram search response.
type NumericBucketingOption struct {
	// BucketBounds: Required. Two adjacent values form a histogram bucket. Values
	// should be in ascending order. For example, if [5, 10, 15] are provided, four
	// buckets are created: (-inf, 5), 5, 10), [10, 15), [15, inf). At most 20
	// [buckets_bound is supported.
	BucketBounds []float64 `json:"bucketBounds,omitempty"`
	// RequiresMinMax: Optional. If set to true, the histogram result includes
	// minimum/maximum value of the numeric field.
	...
	// details.
	ForceSendFields []string `json:"-"`
	// NullFields is a list of field names (e.g. "BucketBounds") to include in API
	// requests with the JSON null value. By default, fields with empty values are
	// omitted from API requests. See
	// https://pkg.go.dev/google.golang.org/api#hdr-NullFields for more details.
	NullFields []string `json:"-"`
...
type NumericBucketingResult struct {
	...
	// MaxValue: Stores the maximum value of the numeric field. Is populated only
	// if [NumericBucketingOption.requires_min_max] is set to true.
	MaxValue float64 `json:"maxValue,omitempty"`
	// MinValue: Stores the minimum value of the numeric field. Will be populated
...
type ProcessingOptions struct {
	// DisableStreetAddressResolution: Optional. If set to `true`, the service does
	...
	// Applied fields are: * description * applicationInfo.instruction * incentives
...
// entity conducting the job search. This information is used to improve the
// performance of the service.
type RequestMetadata struct {
	// DeviceInfo: Optional. The type of device used by the job seeker at the time
	// of the call to the service.
	DeviceInfo *DeviceInfo `json:"deviceInfo,omitempty"`
	// Domain: Required. The client-defined scope or source of the service call,
	// which typically is the domain on which the service has been implemented and
	// is currently being run. For example, if the service is being run by client
	...
	// SessionId: Required. A unique session identification string. A session is
	// defined as the duration of an end user's interaction with the service over a
	// certain period. Obfuscate this field for privacy concerns before providing
	// it to the service. If this field is not available for some reason, send
	// "UNKNOWN". Note that any improvements to the model for a particular tenant
	// site, rely on this field being set correctly to some unique session_id. The
	// maximum number of allowed characters is 255.
	SessionId string `json:"sessionId,omitempty"`
	// UserId: Required. A unique user identification string, as determined by the
	// client. To have the strongest positive impact on search quality make sure
	// the client-level is unique. Obfuscate this field for privacy concerns before
...
type SearchJobsRequest struct {
	// CustomRankingInfo: Optional. Controls over how job documents get ranked on
	// top of existing relevance score (determined by API algorithm).
	CustomRankingInfo *CustomRankingInfo `json:"customRankingInfo,omitempty"`
	// DisableKeywordMatch: Optional. Controls whether to disable exact keyword
	// match on Job.job_title, Job.description, Job.company_display_name,
	// Job.locations, Job.qualifications. When disable keyword match is turned off,
	// a keyword match returns jobs that do not match given category filters when
	...
	// have "program manager" appearing in its description. For queries like
	// "cloud" that does not contain title or location specific ontology, jobs with
	// "cloud" keyword matches are returned regardless of this flag's value. Please
	// use Company.keyword_searchable_custom_fields or
	...
	// DiversificationLevel: Optional. Controls whether highly similar jobs are
	// returned next to each other in the search results. Jobs are identified as
	// highly similar based on their titles, job categories, and locations. Highly
	// similar results are clustered so that only one representative job of the
	// cluster is displayed to the job seeker higher up in the results, with the
	// other jobs being displayed lower down in the results. Defaults to
	...
	// EnableBroadening: Optional. Controls whether to broaden the search when it
	// produces sparse results. Broadened queries append results to the end of the
	// matching results list. Defaults to false.
	EnableBroadening bool `json:"enableBroadening,omitempty"`
	// HistogramFacets: Optional. Histogram requests for jobs matching JobQuery.
	HistogramFacets *HistogramFacets `json:"histogramFacets,omitempty"`
	// HistogramQueries: Optional. Expression based histogram requests for jobs
	// matching JobQuery.
	HistogramQueries []*HistogramQuery `json:"histogramQueries,omitempty"`
	// JobQuery: Optional. Query used to search against jobs, such as keyword,
	...
	// in the search results: Job.name, Job.requisition_id, Job.title,
	...
	// Offset: Optional. An integer that specifies the current offset (that is,
	// starting result location, amongst the jobs deemed by the API as relevant) in
	...
	// means to return results starting from the first matching job, and 10 means
	// to return from the 11th job. This can be used for pagination, (for example,
	// pageSize = 10 and offset = 10 means to return from the second page).
	Offset int64 `json:"offset,omitempty"`
	// OrderBy: Optional. The criteria determining how search results are sorted.
	// Default is "relevance desc". Supported options are: * "relevance desc": By
	...
	// PageSize: Optional. A limit on the number of jobs returned in the search
	// results. Increasing this value above the default value of 10 can increase
	// search response time. The value can be between 1 and 100.
	PageSize int64 `json:"pageSize,omitempty"`
	// PageToken: Optional. The token specifying the current offset within search
	// results. See SearchJobsResponse.next_page_token for an explanation of how to
	...
	// SearchMode: Optional. Mode of a search. Defaults to SearchMode.JOB_SEARCH.
	//
	// Possible values:
...
type SearchJobsResponse struct {
	...
	// EstimatedTotalSize: An estimation of the number of jobs that match the
	// specified query. This number is not guaranteed to be accurate. For accurate
	// results, see SearchJobsResponse.total_size.
...
type UpdateCompanyRequest struct {
	// Company: Required. The company resource to replace the current resource in
	// the system.
	Company *Company `json:"company,omitempty"`
...
type UpdateJobRequest struct {
	...
	Job *Job `json:"job,omitempty"`
	// UpdateMask: Optional but strongly recommended to be provided for the best
	// service experience. If update_mask is provided, only the specified fields in
...
// Complete: Completes the specified prefix with keyword suggestions. Intended
// for use by a job search auto-complete search box.
//
//   - name: Resource name of project the completion is performed within. The
...
// LanguageCode sets the optional parameter "languageCode": Deprecated. Use
// language_codes instead.  The language of the query. This is the BCP-47
// language code, such as "en-US" or "sr-Latn". For more information, see Tags
// for Identifying Languages (https://tools.ietf.org/html/bcp47). For
...
// (https://tools.ietf.org/html/bcp47). For CompletionType.JOB_TITLE type, only
...
// same language_codes are returned. For CompletionType.COMBINED type, only
// open jobs with the same language_codes or companies having open jobs with
// the same language_codes are returned. The maximum number of allowed
// characters is 255.
...
func (c *ProjectsCompleteCall) doRequest(alt string) (*http.Response, error) {
	...
	c.s.logger.DebugContext(c.ctx_, "api request", "serviceName", apiName, "rpcName", "jobs.projects.complete", "request", internallog.HTTPRequest(req, nil))
...
func (c *ProjectsCompleteCall) Do(opts ...googleapi.CallOption) (*CompleteQueryResponse, error) {
	...
	c.s.logger.DebugContext(c.ctx_, "api response", "serviceName", apiName, "rpcName", "jobs.projects.complete", "response", internallog.HTTPResponse(res, b))
	return ret, nil
}
...
func (c *ProjectsClientEventsCreateCall) doRequest(alt string) (*http.Response, error) {
	...
	body, err := googleapi.WithoutDataWrapper.JSONBuffer(c.createclienteventrequest)
	if err != nil {
		return nil, err
	}
	c.urlParams_.Set("alt", alt)
	...
	c.s.logger.DebugContext(c.ctx_, "api request", "serviceName", apiName, "rpcName", "jobs.projects.clientEvents.create", "request", internallog.HTTPRequest(req, body.Bytes()))
	return gensupport.SendRequest(c.ctx_, c.s.client, req)
}
...
func (c *ProjectsClientEventsCreateCall) Do(opts ...googleapi.CallOption) (*ClientEvent, error) {
	...
	c.s.logger.DebugContext(c.ctx_, "api response", "serviceName", apiName, "rpcName", "jobs.projects.clientEvents.create", "response", internallog.HTTPResponse(res, b))
	return ret, nil
}
...
//
//   - parent: Resource name of the project under which the company is created.
...
func (c *ProjectsCompaniesCreateCall) doRequest(alt string) (*http.Response, error) {
	reqHeaders := gensupport.SetHeaders(c.s.userAgent(), "application/json", c.header_)
	body, err := googleapi.WithoutDataWrapper.JSONBuffer(c.createcompanyrequest)
	if err != nil {
		return nil, err
	}
	c.urlParams_.Set("alt", alt)
	...
	c.s.logger.DebugContext(c.ctx_, "api request", "serviceName", apiName, "rpcName", "jobs.projects.companies.create", "request", internallog.HTTPRequest(req, body.Bytes()))
	return gensupport.SendRequest(c.ctx_, c.s.client, req)
}
...
func (c *ProjectsCompaniesCreateCall) Do(opts ...googleapi.CallOption) (*Company, error) {
	...
	c.s.logger.DebugContext(c.ctx_, "api response", "serviceName", apiName, "rpcName", "jobs.projects.companies.create", "response", internallog.HTTPResponse(res, b))
	return ret, nil
}
...
func (c *ProjectsCompaniesDeleteCall) doRequest(alt string) (*http.Response, error) {
	...
	c.s.logger.DebugContext(c.ctx_, "api request", "serviceName", apiName, "rpcName", "jobs.projects.companies.delete", "request", internallog.HTTPRequest(req, nil))
	return gensupport.SendRequest(c.ctx_, c.s.client, req)
}
...
func (c *ProjectsCompaniesDeleteCall) Do(opts ...googleapi.CallOption) (*Empty, error) {
	...
	c.s.logger.DebugContext(c.ctx_, "api response", "serviceName", apiName, "rpcName", "jobs.projects.companies.delete", "response", internallog.HTTPResponse(res, b))
	return ret, nil
}
...
func (c *ProjectsCompaniesGetCall) doRequest(alt string) (*http.Response, error) {
	...
	c.s.logger.DebugContext(c.ctx_, "api request", "serviceName", apiName, "rpcName", "jobs.projects.companies.get", "request", internallog.HTTPRequest(req, nil))
	return gensupport.SendRequest(c.ctx_, c.s.client, req)
}
...
func (c *ProjectsCompaniesGetCall) Do(opts ...googleapi.CallOption) (*Company, error) {
	...
	c.s.logger.DebugContext(c.ctx_, "api response", "serviceName", apiName, "rpcName", "jobs.projects.companies.get", "response", internallog.HTTPResponse(res, b))
	return ret, nil
}
...
func (c *ProjectsCompaniesListCall) doRequest(alt string) (*http.Response, error) {
	...
	c.s.logger.DebugContext(c.ctx_, "api request", "serviceName", apiName, "rpcName", "jobs.projects.companies.list", "request", internallog.HTTPRequest(req, nil))
	return gensupport.SendRequest(c.ctx_, c.s.client, req)
}
...
func (c *ProjectsCompaniesListCall) Do(opts ...googleapi.CallOption) (*ListCompaniesResponse, error) {
	...
	c.s.logger.DebugContext(c.ctx_, "api response", "serviceName", apiName, "rpcName", "jobs.projects.companies.list", "response", internallog.HTTPResponse(res, b))
	return ret, nil
}
...
// Patch: Updates specified company. Company names can't be updated. To update
// a company name, delete the company and all jobs associated with it, and only
// then re-create them.
//
...
func (c *ProjectsCompaniesPatchCall) doRequest(alt string) (*http.Response, error) {
	reqHeaders := gensupport.SetHeaders(c.s.userAgent(), "application/json", c.header_)
	body, err := googleapi.WithoutDataWrapper.JSONBuffer(c.updatecompanyrequest)
	if err != nil {
		return nil, err
	}
	c.urlParams_.Set("alt", alt)
	...
	c.s.logger.DebugContext(c.ctx_, "api request", "serviceName", apiName, "rpcName", "jobs.projects.companies.patch", "request", internallog.HTTPRequest(req, body.Bytes()))
	return gensupport.SendRequest(c.ctx_, c.s.client, req)
}
...
func (c *ProjectsCompaniesPatchCall) Do(opts ...googleapi.CallOption) (*Company, error) {
	...
	c.s.logger.DebugContext(c.ctx_, "api response", "serviceName", apiName, "rpcName", "jobs.projects.companies.patch", "response", internallog.HTTPResponse(res, b))
	return ret, nil
}
...
//
//   - parent: The resource name of the project under which the job is created.
//     The format is "projects/{project_id}", for example,
//     "projects/api-test-project".
...
func (c *ProjectsJobsBatchDeleteCall) doRequest(alt string) (*http.Response, error) {
	...
	c.s.logger.DebugContext(c.ctx_, "api request", "serviceName", apiName, "rpcName", "jobs.projects.jobs.batchDelete", "request", internallog.HTTPRequest(req, body.Bytes()))
	return gensupport.SendRequest(c.ctx_, c.s.client, req)
}
...
func (c *ProjectsJobsBatchDeleteCall) Do(opts ...googleapi.CallOption) (*Empty, error) {
	...
	c.s.logger.DebugContext(c.ctx_, "api response", "serviceName", apiName, "rpcName", "jobs.projects.jobs.batchDelete", "response", internallog.HTTPResponse(res, b))
	return ret, nil
}
...
func (c *ProjectsJobsCreateCall) doRequest(alt string) (*http.Response, error) {
	reqHeaders := gensupport.SetHeaders(c.s.userAgent(), "application/json", c.header_)
	body, err := googleapi.WithoutDataWrapper.JSONBuffer(c.createjobrequest)
	if err != nil {
		return nil, err
	}
	c.urlParams_.Set("alt", alt)
	...
	c.s.logger.DebugContext(c.ctx_, "api request", "serviceName", apiName, "rpcName", "jobs.projects.jobs.create", "request", internallog.HTTPRequest(req, body.Bytes()))
	return gensupport.SendRequest(c.ctx_, c.s.client, req)
}
...
func (c *ProjectsJobsCreateCall) Do(opts ...googleapi.CallOption) (*Job, error) {
	...
	c.s.logger.DebugContext(c.ctx_, "api response", "serviceName", apiName, "rpcName", "jobs.projects.jobs.create", "response", internallog.HTTPResponse(res, b))
	return ret, nil
}
...
func (c *ProjectsJobsDeleteCall) doRequest(alt string) (*http.Response, error) {
	...
	c.s.logger.DebugContext(c.ctx_, "api request", "serviceName", apiName, "rpcName", "jobs.projects.jobs.delete", "request", internallog.HTTPRequest(req, nil))
	return gensupport.SendRequest(c.ctx_, c.s.client, req)
}
...
func (c *ProjectsJobsDeleteCall) Do(opts ...googleapi.CallOption) (*Empty, error) {
	...
	c.s.logger.DebugContext(c.ctx_, "api response", "serviceName", apiName, "rpcName", "jobs.projects.jobs.delete", "response", internallog.HTTPResponse(res, b))
	return ret, nil
}
...
func (c *ProjectsJobsGetCall) doRequest(alt string) (*http.Response, error) {
	...
	c.s.logger.DebugContext(c.ctx_, "api request", "serviceName", apiName, "rpcName", "jobs.projects.jobs.get", "request", internallog.HTTPRequest(req, nil))
	return gensupport.SendRequest(c.ctx_, c.s.client, req)
}
...
func (c *ProjectsJobsGetCall) Do(opts ...googleapi.CallOption) (*Job, error) {
	...
	c.s.logger.DebugContext(c.ctx_, "api response", "serviceName", apiName, "rpcName", "jobs.projects.jobs.get", "response", internallog.HTTPResponse(res, b))
	return ret, nil
}
...
func (c *ProjectsJobsListCall) doRequest(alt string) (*http.Response, error) {
	...
	c.s.logger.DebugContext(c.ctx_, "api request", "serviceName", apiName, "rpcName", "jobs.projects.jobs.list", "request", internallog.HTTPRequest(req, nil))
	return gensupport.SendRequest(c.ctx_, c.s.client, req)
}
...
func (c *ProjectsJobsListCall) Do(opts ...googleapi.CallOption) (*ListJobsResponse, error) {
	...
	c.s.logger.DebugContext(c.ctx_, "api response", "serviceName", apiName, "rpcName", "jobs.projects.jobs.list", "response", internallog.HTTPResponse(res, b))
	return ret, nil
}
...
func (c *ProjectsJobsPatchCall) doRequest(alt string) (*http.Response, error) {
	reqHeaders := gensupport.SetHeaders(c.s.userAgent(), "application/json", c.header_)
	body, err := googleapi.WithoutDataWrapper.JSONBuffer(c.updatejobrequest)
	if err != nil {
		return nil, err
	}
	c.urlParams_.Set("alt", alt)
	...
	c.s.logger.DebugContext(c.ctx_, "api request", "serviceName", apiName, "rpcName", "jobs.projects.jobs.patch", "request", internallog.HTTPRequest(req, body.Bytes()))
	return gensupport.SendRequest(c.ctx_, c.s.client, req)
}
...
func (c *ProjectsJobsPatchCall) Do(opts ...googleapi.CallOption) (*Job, error) {
	...
	c.s.logger.DebugContext(c.ctx_, "api response", "serviceName", apiName, "rpcName", "jobs.projects.jobs.patch", "response", internallog.HTTPResponse(res, b))
	return ret, nil
}
...
//
//   - parent: The resource name of the project to search within. The format is
...
func (c *ProjectsJobsSearchCall) doRequest(alt string) (*http.Response, error) {
	...
	c.s.logger.DebugContext(c.ctx_, "api request", "serviceName", apiName, "rpcName", "jobs.projects.jobs.search", "request", internallog.HTTPRequest(req, body.Bytes()))
	return gensupport.SendRequest(c.ctx_, c.s.client, req)
}
...
func (c *ProjectsJobsSearchCall) Do(opts ...googleapi.CallOption) (*SearchJobsResponse, error) {
	...
	c.s.logger.DebugContext(c.ctx_, "api response", "serviceName", apiName, "rpcName", "jobs.projects.jobs.search", "response", internallog.HTTPResponse(res, b))
	return ret, nil
}
...
// are targeted to passive job seekers. This call constrains the visibility of
...
func (c *ProjectsJobsSearchForAlertCall) doRequest(alt string) (*http.Response, error) {
	...
	c.s.logger.DebugContext(c.ctx_, "api request", "serviceName", apiName, "rpcName", "jobs.projects.jobs.searchForAlert", "request", internallog.HTTPRequest(req, body.Bytes()))
	return gensupport.SendRequest(c.ctx_, c.s.client, req)
}
...
func (c *ProjectsJobsSearchForAlertCall) Do(opts ...googleapi.CallOption) (*SearchJobsResponse, error) {
	...
	c.s.logger.DebugContext(c.ctx_, "api response", "serviceName", apiName, "rpcName", "jobs.projects.jobs.searchForAlert", "response", internallog.HTTPResponse(res, b))
	return ret, nil
}
