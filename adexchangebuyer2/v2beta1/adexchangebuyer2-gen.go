...
func NewBuyersService(s *Service) *BuyersService {
	...
	rs.FilterSets = NewBuyersFilterSetsService(s)
	return rs
}
...
func NewBuyersFilterSetsService(s *Service) *BuyersFilterSetsService {
	rs := &BuyersFilterSetsService{s: s}
	rs.BidMetrics = NewBuyersFilterSetsBidMetricsService(s)
	rs.BidResponseErrors = NewBuyersFilterSetsBidResponseErrorsService(s)
	rs.BidResponsesWithoutBids = NewBuyersFilterSetsBidResponsesWithoutBidsService(s)
	rs.FilteredBidRequests = NewBuyersFilterSetsFilteredBidRequestsService(s)
	rs.FilteredBids = NewBuyersFilterSetsFilteredBidsService(s)
	rs.ImpressionMetrics = NewBuyersFilterSetsImpressionMetricsService(s)
	rs.LosingBids = NewBuyersFilterSetsLosingBidsService(s)
	rs.NonBillableWinningBids = NewBuyersFilterSetsNonBillableWinningBidsService(s)
	return rs
}
...
func NewBuyersFilterSetsBidMetricsService(s *Service) *BuyersFilterSetsBidMetricsService {
	rs := &BuyersFilterSetsBidMetricsService{s: s}
	return rs
}
...
func NewBuyersFilterSetsBidResponseErrorsService(s *Service) *BuyersFilterSetsBidResponseErrorsService {
	rs := &BuyersFilterSetsBidResponseErrorsService{s: s}
	return rs
}
...
func NewBuyersFilterSetsBidResponsesWithoutBidsService(s *Service) *BuyersFilterSetsBidResponsesWithoutBidsService {
	rs := &BuyersFilterSetsBidResponsesWithoutBidsService{s: s}
	return rs
}
...
func NewBuyersFilterSetsFilteredBidRequestsService(s *Service) *BuyersFilterSetsFilteredBidRequestsService {
	rs := &BuyersFilterSetsFilteredBidRequestsService{s: s}
	return rs
}
...
func NewBuyersFilterSetsFilteredBidsService(s *Service) *BuyersFilterSetsFilteredBidsService {
	rs := &BuyersFilterSetsFilteredBidsService{s: s}
	rs.Creatives = NewBuyersFilterSetsFilteredBidsCreativesService(s)
	rs.Details = NewBuyersFilterSetsFilteredBidsDetailsService(s)
	return rs
}
...
func NewBuyersFilterSetsFilteredBidsCreativesService(s *Service) *BuyersFilterSetsFilteredBidsCreativesService {
	rs := &BuyersFilterSetsFilteredBidsCreativesService{s: s}
	return rs
}
...
func NewBuyersFilterSetsFilteredBidsDetailsService(s *Service) *BuyersFilterSetsFilteredBidsDetailsService {
	rs := &BuyersFilterSetsFilteredBidsDetailsService{s: s}
	return rs
}
...
func NewBuyersFilterSetsImpressionMetricsService(s *Service) *BuyersFilterSetsImpressionMetricsService {
	rs := &BuyersFilterSetsImpressionMetricsService{s: s}
	return rs
}
...
func NewBuyersFilterSetsLosingBidsService(s *Service) *BuyersFilterSetsLosingBidsService {
	rs := &BuyersFilterSetsLosingBidsService{s: s}
	return rs
}
...
func NewBuyersFilterSetsNonBillableWinningBidsService(s *Service) *BuyersFilterSetsNonBillableWinningBidsService {
	rs := &BuyersFilterSetsNonBillableWinningBidsService{s: s}
	return rs
}
...
// AbsoluteDateRange: An absolute date range, specified by its start date and
// end date. The supported range of dates begins 30 days before today and ends
// today. Validity checked upon filter set creation. If a filter set with an
...
// representing how many bids with the specified dimension values were
...
// specified dimension values that were considered to have no applicable bids,
...
// values where the corresponding bid request or bid response was not
// successful, as described by the specified callout status.
...
// Client: A client resource represents a client buyer—an agency, a brand, or
// an advertiser customer of the sponsor buyer. Users associated with the
// client buyer have restricted access to the Marketplace and certain other
// sections of the Authorized Buyers UI based on the role granted to the client
...
// access to the Marketplace and certain other sections of the Authorized
// Buyers UI based on the role granted to the associated client buyer. The only
// way a new client user can be created is through accepting an email
...
// the Authorized Buyers UI. All fields are required unless otherwise
// specified.
...
// Programmatic Guaranteed/ Preferred Deal in Ad Manager. This doesn't apply to
// Private Auction and AdX Preferred Deals.
...
// associated with a Programmatic Guaranteed/Preferred Deal in Ad Manager.
...
// that did not win the auction (either were filtered pre-auction or lost the
// auction), as described by the specified creative status.
...
// contains a list of included and excluded numeric IDs.
...
// proposal can contain multiple deals. A deal contains the terms and targeting
...
// is considered paused if either hasBuyerPaused or hasSellPaused is true.
...
// specify things like price per buyer, the type of pricing model (for example,
...
// FilterSet: A set of filters that is applied to a request for data. Within a
// filter set, an AND operation is performed across the filters represented by
// each field. An OR operation is performed across the filters represented by
// the multiple values of a repeated field, for example, "format=VIDEO AND
...
// dimension values that have the specified creative.
...
// dimension values, among those filtered due to the requested filtering reason
// (for example, creative status), that have the specified detail.
...
// excluded mobile application IDs that publishers own. Mobile application IDs
// are from App Store and Google Play Store. Android App ID, for example,
// com.google.android.apps.maps, can be found in Google Play Store URL. iOS App
// ID (which is a number) can be found at the end of iTunes store URL. First
// party mobile applications is either included or excluded.
...
// impressions, representing how many impressions with the specified dimension
...
// targeted on an ad request. It only applies to Private Auction, AdX Preferred
// Deals and Auction Packages. This targeting does not apply to Programmatic
// Guaranteed and Preferred Deals in Ad Manager.
...
// bid responses resulted in an error.
...
// reasons that bid responses were considered to have no applicable bids.
...
// all creatives associated with a given filtered bid reason.
...
// all details associated with a given filtered bid reason.
...
// that bid requests were filtered and not sent to the buyer.
...
// were filtered from the auction.
...
// are measured in number of impressions.
...
// for which a buyer was not billed for a winning bid.
...
// used by advertisers to target ad inventory. For example, they can choose to
// target ad requests only if the user is in the US. Multiple types of
...
// represents a count that may be either exact or estimated (for example, when
// sampled).
...
// specified dimension values for which the buyer was not billed, as described
...
// Auctions can be created only by the seller, but they can be returned in a
// get or list request.
...
// PricePerBuyer in a product can become 0 or 1 deals. To check if there is a
// PricePerBuyer for a particular buyer or buyer/advertiser pair, we look for
// the most specific matching rule - we first look for a rule matching the
...
// is associated with certain terms and targeting information which helps the
...
// timestamp. For filter sets that specify a realtime time range RTB metrics
// continue to be aggregated throughout the lifetime of the filter set.
...
// duration. The supported range of dates begins 30 days before today and ends
// today, for example, the limits for these values are: offset_days >= 0
...
// RowDimensions: A response may include multiple rows, breaking down along
// various dimensions. Encapsulates the values of all dimensions for a given
// row.
...
// a specific context. A context here relates to where something ultimately
// serves (for example, a user or publisher geo, a platform, an HTTPS versus
...
// TargetingCriteria: Advertisers can target different attributes of an ad
// slot. For example, they can choose to show ads only if the user is in the
// U.S. Such targeting criteria can be specified as part of Shared Targeting.
...
// google.com). For Private Auction and AdX Preferred Deals, URLs are either
// included or excluded. For Programmatic Guaranteed and Preferred Deals, this
// doesn't apply.
...
//     buyer is a customer; the sponsor buyer to create a client for. (required).
...
// server may return fewer clients than requested. If unspecified, the server
...
// PartnerClientId sets the optional parameter "partnerClientId": Optional
// unique identifier (from the standpoint of an Ad Exchange sponsor buyer
// partner) of the client to return. If specified, at most one client will be
...
//
//   - accountId: Unique numerical account ID for the buyer of which the client
//     buyer is a customer; the sponsor buyer to update a client for. (required).
...
// Create: Creates and sends out an email invitation to access an Ad Exchange
...
// Any non-2xx status code is an error. Response headers are in either
// *ClientUserInvitation.ServerResponse.Header or (if a response was returned
...
// List: Lists all the client users invitations for a client with a given
// account ID.
//
//   - accountId: Numerical account ID of the client's sponsor buyer. (required).
...
// may return fewer clients than requested. If unspecified, server will pick an
// appropriate default.
...
// Any non-2xx status code is an error. Response headers are in either
// *ListClientUserInvitationsResponse.ServerResponse.Header or (if a response
...
//
//   - accountId: Numerical account ID of the client's sponsor buyer. (required).
//   - clientAccountId: Numerical account ID of the client buyer that the user to
//     be retrieved is associated with. (required).
...
// List: Lists all the known client users for a specified sponsor buyer account
// ID.
//
//   - accountId: Numerical account ID of the sponsor buyer of the client to list
//     users for. (required).
...
// Update: Updates an existing client user. Only the user status can be changed
// on update.
//
//   - accountId: Numerical account ID of the client's sponsor buyer. (required).
//   - clientAccountId: Numerical account ID of the client buyer that the user to
...
// DuplicateIdMode sets the optional parameter "duplicateIdMode": Indicates if
// multiple creatives can share an ID or not. Default is NO_DUPLICATES (one ID
// per creative).
//
// Possible values:
//
...
//
// allow multiple creatives to share the same ID. Get and Update requests will
// not be possible for any ID that has more than one creative associated. (List
// will still function.) This is only intended for backwards compatibility in
// cases where a single ID is already shared by multiple creatives from
// previous APIs.
...
//
//   - accountId: The account to list the creatives from. Specify "-" to list all
...
// Query sets the optional parameter "query": An optional query string to
// filter creatives. If no filter is specified, all active creatives will be
// returned. Supported queries are: - accountId=*account_id_string* -
// creativeId=*creative_id_string* - dealsStatus: {approved,
...
// numeric attribute from the list of attributes} - disapprovalReason: {a
// reason from DisapprovalReason} Example: 'accountId=12345 AND
...
// StopWatching: Stops watching a creative. Will stop push notifications being
...
// Watch: Watches a creative. Will result in push notifications being sent to
...
//
//   - accountId: The account to list the associations from. Specify "-" to list
//     all creatives the current user has access to.
//   - creativeId: The creative ID to list the associations from. Specify "-" to
//     list all creatives under the above account.
...
// PageSize sets the optional parameter "pageSize": Requested page size. Server
// may return fewer associations than requested. If unspecified, server will
...
// Query sets the optional parameter "query": An optional query string to
// filter deal associations. If no filter is specified, all associations will
...
// Any non-2xx status code is an error. Response headers are in either
// *ListDealAssociationsResponse.ServerResponse.Header or (if a response was
// returned at all) in error.(*googleapi.Error).Header. Use
// googleapi.IsNotModified to check whether the returned error was because
// http.StatusNotModified was returned.
...
// renegotiated. A filter expression (PQL query) may be specified to filter the
// results. The notes will not be returned.
//
...
// Filter sets the optional parameter "filter": An optional PQL filter query
// used to query for proposals. Nested repeated fields, such as
...
// FilterSyntax sets the optional parameter "filterSyntax": Syntax the filter
// is written in. Current implementation defaults to PQL but in the future it
// will be LIST_FILTER.
//
// Possible values:
//
...
// Resume: Update given deals to resume serving. This method will set the
...
// List: List all products visible to the buyer (optionally filtered by the
// specified PQL query).
//
// - accountId: Account ID of the buyer.
...
// Filter sets the optional parameter "filter": An optional PQL query used to
// query for products. See
...
// Accept: Mark the proposal as accepted at the given revision number. If the
...
// CancelNegotiation: Cancel an ongoing negotiation on a proposal. This does
// not cancel or end serving for the deals if the proposal has been finalized,
// but only cancels a negotiation unilaterally.
//
// - accountId: Account ID of the buyer.
...
// CompleteSetup: You can opt-in to manually update proposals to indicate that
// setup is complete. By default, proposal setup is automatically completed
// after their deals are finalized. Contact your Technical Account Manager to
// opt in. Buyers can call this method when the proposal has been finalized,
// and all the required creatives have been uploaded using the Creatives API.
// This call updates the `is_setup_completed` field on the deals in the
// proposal, and notifies the seller. The server then advances the revision
// number of the most recent proposal. To mark an individual deal as ready to
...
// Create: Create the given proposal. Each created proposal and any deals it
...
// Get: Gets a proposal given its ID. The proposal is returned at its head
// revision.
//
// - accountId: Account ID of the buyer.
...
// List: List proposals. A filter expression (PQL query) may be specified to
...
// that Bidder/ChildSeat relationships differ from the usual behavior. A Bidder
...
// Pause: Update the given proposal to pause serving. This method will set the
// `DealServingMetadata.DealPauseStatus.has_buyer_paused` bit to true for all
...
// Resume: Update the given proposal to resume serving. This method will set
// the `DealServingMetadata.DealPauseStatus.has_buyer_paused` bit to false for
// all deals in the proposal. Note that if the `has_seller_paused` bit is also
// set, serving will not resume until the seller also resumes. It is a no-op to
...
// Update: Update the given proposal at the client known revision number. If
...
// unique ID and create the deal. If passed-in deal has a `deal_id`, it will be
// updated to match the passed-in copy. Any existing deals not present in the
...
// PageSize sets the optional parameter "pageSize": Specify the number of
...
// PageToken sets the optional parameter "pageToken": The page token as return
// from ListPublisherProfilesResponse.
...
// Create: Creates the specified filter set for the account with the given
// account ID.
//
//   - ownerName: Name of the owner (bidder or account) of the filter set to be
//     created. For example: - For a bidder-level filter set for bidder 123:
...
// IsTransient sets the optional parameter "isTransient": Whether the filter
// set is transient, or should be persisted indefinitely. By default, filter
// sets are not transient. If transient, it will be available for at least 1
// hour after creation.
...
// Any non-2xx status code is an error. Response headers are in either
// *FilterSet.ServerResponse.Header or (if a response was returned at all) in
// error.(*googleapi.Error).Header. Use googleapi.IsNotModified to check
// whether the returned error was because http.StatusNotModified was returned.
...
// Delete: Deletes the requested filter set from the account with the given
// account ID.
//
...
// Get: Retrieves the requested filter set for the account with the given
// account ID.
//
...
//
//   - ownerName: Name of the owner (bidder or account) of the filter sets to be
//     listed. For example: - For a bidder-level filter set for bidder 123:
//     `bidders/123` - For an account-level filter set for the buyer account
//     representing bidder 123: `bidders/123/accounts/123` - For an account-level
//     filter set for the child seat buyer account 456 whose bidder is 123:
//     `bidders/123/accounts/456`.
...
// Any non-2xx status code is an error. Response headers are in either
// *ListFilterSetsResponse.ServerResponse.Header or (if a response was returned
// at all) in error.(*googleapi.Error).Header. Use googleapi.IsNotModified to
// check whether the returned error was because http.StatusNotModified was
// returned.
...
// Any non-2xx status code is an error. Response headers are in either
// *ListBidMetricsResponse.ServerResponse.Header or (if a response was returned
// at all) in error.(*googleapi.Error).Header. Use googleapi.IsNotModified to
// check whether the returned error was because http.StatusNotModified was
// returned.
...
// List: List all errors that occurred in bid responses, with the number of bid
...
// Any non-2xx status code is an error. Response headers are in either
// *ListBidResponseErrorsResponse.ServerResponse.Header or (if a response was
// returned at all) in error.(*googleapi.Error).Header. Use
// googleapi.IsNotModified to check whether the returned error was because
// http.StatusNotModified was returned.
...
// List: List all reasons for which bid responses were considered to have no
...
// Any non-2xx status code is an error. Response headers are in either
// *ListBidResponsesWithoutBidsResponse.ServerResponse.Header or (if a response
// was returned at all) in error.(*googleapi.Error).Header. Use
// googleapi.IsNotModified to check whether the returned error was because
// http.StatusNotModified was returned.
...
// List: List all reasons that caused a bid request not to be sent for an
...
// Any non-2xx status code is an error. Response headers are in either
// *ListFilteredBidRequestsResponse.ServerResponse.Header or (if a response was
// returned at all) in error.(*googleapi.Error).Header. Use
// googleapi.IsNotModified to check whether the returned error was because
// http.StatusNotModified was returned.
...
// List: List all reasons for which bids were filtered, with the number of bids
...
// List: List all creatives associated with a specific reason for which bids
...
//     breakdown by creative. See creative-status-codes
//     (https://developers.google.com/authorized-buyers/rtb/downloads/creative-status-codes).
//   - filterSetName: Name of the filter set that should be applied to the
//     requested metrics. For example: - For a bidder-level filter set for bidder
//     123: `bidders/123/filterSets/abc` - For an account-level filter set for
...
// Any non-2xx status code is an error. Response headers are in either
// *ListCreativeStatusBreakdownByCreativeResponse.ServerResponse.Header or (if
...
// List: List all details associated with a specific reason for which bids were
// filtered, with the number of bids filtered for each detail.
//
//   - creativeStatusId: The ID of the creative status for which to retrieve a
//     breakdown by detail. See creative-status-codes
//     (https://developers.google.com/authorized-buyers/rtb/downloads/creative-status-codes).
...
// Any non-2xx status code is an error. Response headers are in either
// *ListCreativeStatusBreakdownByDetailResponse.ServerResponse.Header or (if a
...
// Any non-2xx status code is an error. Response headers are in either
// *ListImpressionMetricsResponse.ServerResponse.Header or (if a response was
// returned at all) in error.(*googleapi.Error).Header. Use
// googleapi.IsNotModified to check whether the returned error was because
// http.StatusNotModified was returned.
...
// List: List all reasons for which bids lost in the auction, with the number
...
// Any non-2xx status code is an error. Response headers are in either
// *ListLosingBidsResponse.ServerResponse.Header or (if a response was returned
// at all) in error.(*googleapi.Error).Header. Use googleapi.IsNotModified to
// check whether the returned error was because http.StatusNotModified was
// returned.
...
// List: List all reasons for which winning bids were not billable, with the
...
// Any non-2xx status code is an error. Response headers are in either
// *ListNonBillableWinningBidsResponse.ServerResponse.Header or (if a response
// was returned at all) in error.(*googleapi.Error).Header. Use
// googleapi.IsNotModified to check whether the returned error was because
// http.StatusNotModified was returned.
...
func (r *BuyersFilterSetsService) Create(ownerName string, filterset *FilterSet) *BuyersFilterSetsCreateCall {
	c := &BuyersFilterSetsCreateCall{s: r.s, urlParams_: make(gensupport.URLParams)}
	c.ownerName = ownerName
	c.filterset = filterset
	return c
}
...
func (r *BuyersFilterSetsService) Delete(name string) *BuyersFilterSetsDeleteCall {
	c := &BuyersFilterSetsDeleteCall{s: r.s, urlParams_: make(gensupport.URLParams)}
	c.name = name
	return c
}
...
func (r *BuyersFilterSetsService) Get(name string) *BuyersFilterSetsGetCall {
	c := &BuyersFilterSetsGetCall{s: r.s, urlParams_: make(gensupport.URLParams)}
	c.name = name
	return c
}
...
func (c *BuyersFilterSetsGetCall) doRequest(alt string) (*http.Response, error) {
	reqHeaders := gensupport.SetHeaders(c.s.userAgent(), "", c.header_)
	if c.ifNoneMatch_ != "" {
		reqHeaders.Set("If-None-Match", c.ifNoneMatch_)
	}
...
func (r *BuyersFilterSetsService) List(ownerName string) *BuyersFilterSetsListCall {
	c := &BuyersFilterSetsListCall{s: r.s, urlParams_: make(gensupport.URLParams)}
	c.ownerName = ownerName
	return c
}
...
func (c *BuyersFilterSetsListCall) doRequest(alt string) (*http.Response, error) {
	reqHeaders := gensupport.SetHeaders(c.s.userAgent(), "", c.header_)
	if c.ifNoneMatch_ != "" {
		reqHeaders.Set("If-None-Match", c.ifNoneMatch_)
	}
...
func (r *BuyersFilterSetsBidMetricsService) List(filterSetName string) *BuyersFilterSetsBidMetricsListCall {
	c := &BuyersFilterSetsBidMetricsListCall{s: r.s, urlParams_: make(gensupport.URLParams)}
	c.filterSetName = filterSetName
	return c
}
...
func (c *BuyersFilterSetsBidMetricsListCall) doRequest(alt string) (*http.Response, error) {
	reqHeaders := gensupport.SetHeaders(c.s.userAgent(), "", c.header_)
	if c.ifNoneMatch_ != "" {
		reqHeaders.Set("If-None-Match", c.ifNoneMatch_)
	}
...
func (r *BuyersFilterSetsBidResponseErrorsService) List(filterSetName string) *BuyersFilterSetsBidResponseErrorsListCall {
	c := &BuyersFilterSetsBidResponseErrorsListCall{s: r.s, urlParams_: make(gensupport.URLParams)}
	c.filterSetName = filterSetName
	return c
}
...
func (c *BuyersFilterSetsBidResponseErrorsListCall) doRequest(alt string) (*http.Response, error) {
	reqHeaders := gensupport.SetHeaders(c.s.userAgent(), "", c.header_)
	if c.ifNoneMatch_ != "" {
		reqHeaders.Set("If-None-Match", c.ifNoneMatch_)
	}
...
func (r *BuyersFilterSetsBidResponsesWithoutBidsService) List(filterSetName string) *BuyersFilterSetsBidResponsesWithoutBidsListCall {
	c := &BuyersFilterSetsBidResponsesWithoutBidsListCall{s: r.s, urlParams_: make(gensupport.URLParams)}
	c.filterSetName = filterSetName
	return c
}
...
func (c *BuyersFilterSetsBidResponsesWithoutBidsListCall) doRequest(alt string) (*http.Response, error) {
	reqHeaders := gensupport.SetHeaders(c.s.userAgent(), "", c.header_)
	if c.ifNoneMatch_ != "" {
		reqHeaders.Set("If-None-Match", c.ifNoneMatch_)
	}
...
func (r *BuyersFilterSetsFilteredBidRequestsService) List(filterSetName string) *BuyersFilterSetsFilteredBidRequestsListCall {
	c := &BuyersFilterSetsFilteredBidRequestsListCall{s: r.s, urlParams_: make(gensupport.URLParams)}
	c.filterSetName = filterSetName
	return c
}
...
func (c *BuyersFilterSetsFilteredBidRequestsListCall) doRequest(alt string) (*http.Response, error) {
	reqHeaders := gensupport.SetHeaders(c.s.userAgent(), "", c.header_)
	if c.ifNoneMatch_ != "" {
		reqHeaders.Set("If-None-Match", c.ifNoneMatch_)
	}
...
func (r *BuyersFilterSetsFilteredBidsService) List(filterSetName string) *BuyersFilterSetsFilteredBidsListCall {
	c := &BuyersFilterSetsFilteredBidsListCall{s: r.s, urlParams_: make(gensupport.URLParams)}
	c.filterSetName = filterSetName
	return c
}
...
func (c *BuyersFilterSetsFilteredBidsListCall) doRequest(alt string) (*http.Response, error) {
	reqHeaders := gensupport.SetHeaders(c.s.userAgent(), "", c.header_)
	if c.ifNoneMatch_ != "" {
		reqHeaders.Set("If-None-Match", c.ifNoneMatch_)
	}
...
func (r *BuyersFilterSetsFilteredBidsCreativesService) List(filterSetName string, creativeStatusId int64) *BuyersFilterSetsFilteredBidsCreativesListCall {
	c := &BuyersFilterSetsFilteredBidsCreativesListCall{s: r.s, urlParams_: make(gensupport.URLParams)}
	c.filterSetName = filterSetName
	c.creativeStatusId = creativeStatusId
	return c
}
...
func (c *BuyersFilterSetsFilteredBidsCreativesListCall) doRequest(alt string) (*http.Response, error) {
	reqHeaders := gensupport.SetHeaders(c.s.userAgent(), "", c.header_)
	if c.ifNoneMatch_ != "" {
		reqHeaders.Set("If-None-Match", c.ifNoneMatch_)
	}
...
func (r *BuyersFilterSetsFilteredBidsDetailsService) List(filterSetName string, creativeStatusId int64) *BuyersFilterSetsFilteredBidsDetailsListCall {
	c := &BuyersFilterSetsFilteredBidsDetailsListCall{s: r.s, urlParams_: make(gensupport.URLParams)}
	c.filterSetName = filterSetName
	c.creativeStatusId = creativeStatusId
	return c
}
...
func (c *BuyersFilterSetsFilteredBidsDetailsListCall) doRequest(alt string) (*http.Response, error) {
	reqHeaders := gensupport.SetHeaders(c.s.userAgent(), "", c.header_)
	if c.ifNoneMatch_ != "" {
		reqHeaders.Set("If-None-Match", c.ifNoneMatch_)
	}
...
func (r *BuyersFilterSetsImpressionMetricsService) List(filterSetName string) *BuyersFilterSetsImpressionMetricsListCall {
	c := &BuyersFilterSetsImpressionMetricsListCall{s: r.s, urlParams_: make(gensupport.URLParams)}
	c.filterSetName = filterSetName
	return c
}
...
func (c *BuyersFilterSetsImpressionMetricsListCall) doRequest(alt string) (*http.Response, error) {
	reqHeaders := gensupport.SetHeaders(c.s.userAgent(), "", c.header_)
	if c.ifNoneMatch_ != "" {
		reqHeaders.Set("If-None-Match", c.ifNoneMatch_)
	}
...
func (r *BuyersFilterSetsLosingBidsService) List(filterSetName string) *BuyersFilterSetsLosingBidsListCall {
	c := &BuyersFilterSetsLosingBidsListCall{s: r.s, urlParams_: make(gensupport.URLParams)}
	c.filterSetName = filterSetName
	return c
}
...
func (c *BuyersFilterSetsLosingBidsListCall) doRequest(alt string) (*http.Response, error) {
	reqHeaders := gensupport.SetHeaders(c.s.userAgent(), "", c.header_)
	if c.ifNoneMatch_ != "" {
		reqHeaders.Set("If-None-Match", c.ifNoneMatch_)
	}
...
func (r *BuyersFilterSetsNonBillableWinningBidsService) List(filterSetName string) *BuyersFilterSetsNonBillableWinningBidsListCall {
	c := &BuyersFilterSetsNonBillableWinningBidsListCall{s: r.s, urlParams_: make(gensupport.URLParams)}
	c.filterSetName = filterSetName
	return c
}
...
func (c *BuyersFilterSetsNonBillableWinningBidsListCall) doRequest(alt string) (*http.Response, error) {
	reqHeaders := gensupport.SetHeaders(c.s.userAgent(), "", c.header_)
	if c.ifNoneMatch_ != "" {
		reqHeaders.Set("If-None-Match", c.ifNoneMatch_)
	}
