...
// Service Perimeters belonging to an Access Policy.
...
// request. Conditions based on information about the source of the request.
// Note that if the destination of the request is also protected by a
...
// requests based on `egress_from` and `egress_to` stanzas. For an EgressPolicy
...
// ServicePerimeter boundary. For example, an EgressPolicy can be used to allow
// VMs on networks within the ServicePerimeter to access a defined set of
// projects outside the perimeter in certain contexts (e.g. to read data from a
// Cloud Storage bucket or query against a BigQuery dataset). EgressPolicies
...
// the descriptions of EgressFrom and EgressTo.
...
// request. Conditions are based on information about the ApiOperation intended
// to be performed on the `resources` specified. Note that if the destination
...
// this request to succeed. The request must match `operations` AND `resources`
...
// request. Conditions are based on information about the source of the
// request. The request must satisfy what is defined in `sources` AND identity
// related fields in order to match.
...
// request. Conditions are based on information about the ApiOperation intended
// to be performed on the target resource of the request. The request must
// satisfy what is defined in `operations` AND `resources` in order to match.
...
// will be put inside of Operation.response field.
...
// Perimeters in an Access Policy with the Service Perimeters provided. This is
// done atomically.
...
// Regular Service Perimeters cannot overlap, a single Google Cloud project or
// VPC network can only belong to a single regular Service Perimeter. Service
...
// Create: Creates an access policy. This method fails if the organization
// already has an access policy. The long-running operation has a successful
// status after the access policy propagates to long-lasting storage. Syntactic
// and basic semantic errors are returned in `metadata` as a BadRequest proto.
...
// long-running operation has a successful status after the access policy is
...
// Patch: Updates an access policy. The long-running operation from this RPC
...
// access policy. This method replaces the existing IAM policy on the access
// policy. The IAM policy controls the set of users who can perform specific
...
// Create: Creates an access level. The long-running operation from this RPC
// has a successful status after the access level propagates to long-lasting
...
// Delete: Deletes an access level based on the resource name. The long-running
...
// Patch: Updates an access level. The long-running operation from this RPC has
// a successful status after the changes to the access level propagate to
...
// ReplaceAll: Replaces all existing access levels in an access policy with the
// access levels provided. This is done atomically. The long-running operation
...
// Create: Creates an authorized orgs desc. The long-running operation from
// this RPC has a successful status after the authorized orgs desc propagates
// to long-lasting storage. If a authorized orgs desc contains errors, an error
// response is returned for the first error encountered. The name of this
// `AuthorizedOrgsDesc` will be assigned during creation.
//
//   - parent: Resource name for the access policy which owns this Authorized
...
// Delete: Deletes an authorized orgs desc based on the resource name. The
...
// Any non-2xx status code is an error. Response headers are in either
// *AuthorizedOrgsDesc.ServerResponse.Header or (if a response was returned at
...
//
//   - parent: Resource name for the access policy to list Authorized Orgs Desc
...
// PageSize sets the optional parameter "pageSize": Number of Authorized Orgs
// Descs to include in the list. Default 100.
...
// PageToken sets the optional parameter "pageToken": Next page token for the
// next batch of Authorized Orgs Desc instances. Defaults to the first page of
// results.
...
// Any non-2xx status code is an error. Response headers are in either
// *ListAuthorizedOrgsDescsResponse.ServerResponse.Header or (if a response was
// returned at all) in error.(*googleapi.Error).Header. Use
// googleapi.IsNotModified to check whether the returned error was because
// http.StatusNotModified was returned.
...
// Patch: Updates an authorized orgs desc. The long-running operation from this
...
// Commit: Commits the dry-run specification for all the service perimeters in
// an access policy. A commit operation on a service perimeter involves copying
...
//
//   - parent: Resource name for the parent Access Policy which owns all Service
...
// Create: Creates a service perimeter. The long-running operation from this
// RPC has a successful status after the service perimeter propagates to
...
// Patch: Updates a service perimeter. The long-running operation from this RPC
...
// ReplaceAll: Replace all existing service perimeters in an access policy with
// the service perimeters provided. This is done atomically. The long-running
...
// Create: Creates a GcpUserAccessBinding. If the client specifies a name, the
...
// signify that the new binding is deployed onto all affected users, which may
// take more time.
//
...
// Delete: Deletes a GcpUserAccessBinding. Completion of this long-running
// operation does not necessarily signify that the binding deletion is deployed
...
// Any non-2xx status code is an error. Response headers are in either
// *GcpUserAccessBinding.ServerResponse.Header or (if a response was returned
// at all) in error.(*googleapi.Error).Header. Use googleapi.IsNotModified to
// check whether the returned error was because http.StatusNotModified was
// returned.
...
// return. The server may return fewer items. If left blank, the server may
...
// PageToken sets the optional parameter "pageToken": If left blank, returns
// the first page. To enumerate all items, use the next_page_token from your
// previous list operation.
...
// Any non-2xx status code is an error. Response headers are in either
// *ListGcpUserAccessBindingsResponse.ServerResponse.Header or (if a response
...
// Patch: Updates a GcpUserAccessBinding. Completion of this long-running
// operation does not necessarily signify that the changed binding is deployed
// onto all affected users, which may take more time.
//
//   - name: Immutable. Assigned by the server during creation. The last segment
//     has an arbitrary length and has only URI unreserved characters (as defined
//     by RFC 3986 Section 2.3
...
// Any non-2xx status code is an error. Response headers are in either
// *SupportedService.ServerResponse.Header or (if a response was returned at
// all) in error.(*googleapi.Error).Header. Use googleapi.IsNotModified to
// check whether the returned error was because http.StatusNotModified was
// returned.
...
// Any non-2xx status code is an error. Response headers are in either
// *ListSupportedServicesResponse.ServerResponse.Header or (if a response was
// returned at all) in error.(*googleapi.Error).Header. Use
// googleapi.IsNotModified to check whether the returned error was because
// http.StatusNotModified was returned.
