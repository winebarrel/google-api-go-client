...
type Explanation struct {
	// MatchedPermissions: The map from roles to their included permission matching
	// the permission query (e.g. containing `policy.role.permissions:`). Example
...
type IamPolicySearchResult struct {
	...
	// additional information that explains why the search result matches the
	// query.
	Explanation *Explanation `json:"explanation,omitempty"`
	// Policy: The IAM policy attached to the specified resource. Note that the
	...
	// constraint on policies (e.g. an empty query), this contains all the
	// bindings.
	...
	// the resource associated with this IAM policy.
	Resource string `json:"resource,omitempty"`
...
type StandardResourceMetadata struct {
	...
	// (lhttps://cloud.google.com/blog/products/gcp/labelling-and-grouping-your-goog
	// le-cloud-platform-resources) for more information.
...
// SearchAll: Searches all the IAM policies within a given accessible Resource
// Manager scope (project/folder/organization). This RPC gives callers
// especially administrators the ability to search all the IAM policies within
// a scope, even if they don't have `.getIamPolicy` permission of all the IAM
// policies. Callers should have `cloudasset.assets.searchAllIamPolicies`
...
//     resources within the `scope`. The allowed value must be: * Organization
//     number (such as "organizations/123") * Folder number (such as
...
// given. If set to zero, server will pick an appropriate default. Returned
// results may be fewer than requested. When this happens, there could be more
...
// Query sets the optional parameter "query": The query statement. Examples: *
...
// SearchAll: Searches all the resources within a given accessible Resource
// Manager scope (project/folder/organization). This RPC gives callers
// especially administrators the ability to search all the resources within a
// scope, even if they don't have `.get` permission of all the resources.
// Callers should have `cloudasset.assets.searchAllResources` permission on the
...
// that this request searches for. If empty, it will search all the supported
// asset types.
