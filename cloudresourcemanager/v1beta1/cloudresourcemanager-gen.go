...
// OrganizationId sets the optional parameter "organizationId": The id of the
// Organization resource to fetch. This field is deprecated and will be removed
...
// List: Lists Organization resources that are visible to the user and satisfy
...
// Organizations to return in the response. This field is optional.
...
// returned from a previous call to `ListOrganizations` that indicates from
...
func (c *OrganizationsListCall) doRequest(alt string) (*http.Response, error) {
	...
	req.Header = reqHeaders
	c.s.logger.DebugContext(c.ctx_, "api request", "serviceName", apiName, "rpcName", "cloudresourcemanager.organizations.list", "request", internallog.HTTPRequest(req, nil))
	return gensupport.SendRequest(c.ctx_, c.s.client, req)
}
...
func (c *OrganizationsListCall) Do(opts ...googleapi.CallOption) (*ListOrganizationsResponse, error) {
	...
	c.s.logger.DebugContext(c.ctx_, "api response", "serviceName", apiName, "rpcName", "cloudresourcemanager.organizations.list", "response", internallog.HTTPResponse(res, b))
	return ret, nil
}
...
func (c *OrganizationsUpdateCall) doRequest(alt string) (*http.Response, error) {
	reqHeaders := gensupport.SetHeaders(c.s.userAgent(), "application/json", c.header_)
	body, err := googleapi.WithoutDataWrapper.JSONBuffer(c.organization)
	if err != nil {
		return nil, err
	}
	c.urlParams_.Set("alt", alt)
	...
	c.s.logger.DebugContext(c.ctx_, "api request", "serviceName", apiName, "rpcName", "cloudresourcemanager.organizations.update", "request", internallog.HTTPRequest(req, body.Bytes()))
	return gensupport.SendRequest(c.ctx_, c.s.client, req)
}
...
func (c *OrganizationsUpdateCall) Do(opts ...googleapi.CallOption) (*Organization, error) {
	...
	c.s.logger.DebugContext(c.ctx_, "api response", "serviceName", apiName, "rpcName", "cloudresourcemanager.organizations.update", "response", internallog.HTTPResponse(res, b))
	return ret, nil
}
...
// Create: Creates a Project resource. Initially, the Project resource is owned
// by its creator exclusively. The creator can later grant permission to others
// to read or update the Project. Several APIs are activated automatically for
// the Project, including Google Cloud Storage. The parent is identified by a
...
// UseLegacyStack sets the optional parameter "useLegacyStack": A now unused
// experiment opt-out option.
...
// `parent.id` Some examples of using labels as filters: | Filter | Description
// | |------------------|-----------------------------------------------------|
...
// `color` has the value `red` or its label `size` has the value `big`. | If no
// filter is specified, the call will return projects for which the user has
// the `resourcemanager.projects.get` permission. NOTE: To perform a by-parent
// query (eg., what projects are directly in a Folder), the caller must have
// the `resourcemanager.projects.list` permission on the parent and the filter
// must contain both a `parent.type` and a `parent.id` restriction (example:
// "parent.type:folder parent.id:123"). In this case an alternate search index
...
// sent only using the Cloud Platform Console. + Membership changes that leave
// the project without any owners that have accepted the Terms of Service (ToS)
// will be rejected. + If the project is not part of an organization, there
